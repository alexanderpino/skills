...
1. **The ramp is not keyed to the curve it is drawn from — 0.71° on a circle.**
   `d = A(x_s(y) − x)^(2/3)` has contours that are `x`-translates of the shoreline, and a translate
   is not a normal offset, so a ray arriving normal to the shoreline does not stay normal to the
   contours it crosses and refraction returns some obliquity. On a *circular* bay at exactly radial
   incidence, rebuilding the same shoreline with `d = A(R_s − r)^(2/3)`, `r` measured from the pole
   — concentric arcs, on which a radial ray is normal to every contour — removes exactly this much.
   **This number does not transfer to the built bay, and the next subsection is why.**
2. **The march meeting curvature — 1.46° on the same circle.** A column-marched transform carries
   the ray's alongshore drift only through `∂k/∂y`. On *straight* contours at the same 20° obliquity
   the identical march leaves 0.20°; on concentric contours with exactly radial incidence it leaves
   1.66°. The excess is the solver, not the physics, and it is named rather than absorbed into a
   tolerance.

### What "cross-shore distance" means on a curved coast — and why an axis offset is not a normal offset

The Dean profile is `h ∝ x^(2/3)`, and terrain-architect `12` states it as *"depth ∝
distance^⅔ … a graded ramp from shoreline to shelf break"*. **Neither chapter says what the
distance is measured along**, and on a straight coast there is nothing to say. On a curved one there
are two different surfaces hiding in the same sentence, and they are generated by two different
flows. **`D`**

```
axis keying     s_a(x, y) = x_s(y) − x            the family of TRANSLATES of the shoreline
normal keying   s_n(P)    = dist(P, shoreline)    the family of NORMAL OFFSETS (parallel curves)
```

**They coincide if and only if `φ_s ≡ atan(dx_s/dy)` is identically zero** — the shore parallel to
the grid's alongshore axis. Every scene this project rendered before it built a bay satisfied that,
which is why eight waves of surf work could not see the difference.

Three consequences, in the order they matter to a renderer.

**1 · The translate family crowds its contours, and the crowding is an artefact of the grid.** Two
translates separated by `Δs` along `x` are separated by `Δs·cos φ_s` *perpendicular*, so the
depth-gradient magnitude of an axis-keyed ramp varies alongshore purely with the shore's obliquity
to the grid. Measured on the reference bay, the perpendicular gap between the 2 m and 6 m contours
runs 239.5–253.2 m — a **5.4 %** crowding where the Dean offset itself is a constant 253.2 m
(`s10-bathy-contours.png`; **`M`**). Rotate the grid and the bathymetry changes. That alone
disqualifies it as a statement about a landform.

**2 · A normal offset shares its normal lines with the curve it offsets, and that is the whole
mechanism.** It is the classical property of parallel curves: the normal to the offset at the foot
point is the normal to the original at the same foot point, so a ray travelling along a shoreline
normal is normal to **every** contour it crosses and Snell is the identity along it — it arrives at
the break point with the obliquity it was launched with, and no more. The translate family has no
such property. A ray launched normal at station `y` reaches, after travelling `s`, the contour
belonging to station `y + s·sin φ_s`, where the contour normal has turned by

```
Δθ(s) = −(dφ_s/dy) · s · sin(φ_s) + O(s²)
```

— **first order in the shoreline curvature × the offset × the sine of the shore's own obliquity to
the grid.** It vanishes on a straight coast (curvature zero) and on a coast parallel to the grid
(`sin φ_s` zero), and on nothing else. Measured off the reference bed's own gradient at the 2 m
contour: **0.397°** mean `|Δθ|` under axis keying, **0.0008°** under normal keying, against
**0.397°** from the formula above. Derived and measured to three figures. **`D`, checked `M`.**

**3 · The concentric ramp is the special case of the normal offset for a circular shore.** If the
shoreline is a circular arc about the pole, its normal offsets *are* the concentric arcs, and
"a radial ray stays radial" is the shared-normal property with the pole substituted in. The normal
offset is the same statement for a shore of any shape, **with no pole in it at all** — which
matters, because the built bay is a rock headland, a log spiral and a straight tangential beach, and
only the middle third of it is an arc about anything.

**The limit is real and it is the medial axis.** Normal offsets of a concave curve fold at the
curve's centres of curvature; past that locus the nearest-point map is many-to-one and `min` puts a
*crease* — a gradient discontinuity, not a step — in the bed. Measure the share of ramp cells with
`|∇ dist| < 1` and you have the fold as a number: **0 %** on the analytic bay, **0.25 %** on the
same scene's rough rock shoreline, whose 380 m-correlation roughness gives it a 90 m minimum radius
of curvature well inside a 483 m ramp. So the rule is *key the ramp normally to the plan-form you
stated*, and a shoreline too rough to have a single-valued offset field inside its own ramp is
telling you the ramp is not the right description of it. **`M`**

### The attribution that failed, and the statistic that caused it

The concentric ramp above was priced at **0.71°** and the price was measured on the δ = 0 circle at
exactly radial incidence. Applied to the bay it was written against it removes **0.041°** of the
2.801° whole-domain mean and **0.252°** over the spiral span. It reproduces exactly on the circle
(0.7103°), so the number was right and the **attribution** was wrong. Two independent reasons, both
measured: **`M`**

**(a) The statistic is not additive.** Every figure in the attribution is a mean of `|θ_loc|`. The
keying error is *antisymmetric about the bay's apex* — `sin φ_s` changes sign there, so `Δθ` does —
and therefore carries almost no alongshore mean: on the circle the two keyings differ by 0.710° in
`mean|θ|` and by 0.05° in the **signed** mean. `mean|·|` of a zero-mean term does not add to
`mean|·|` of a biased one. The moment the bay carries the +1.4° drift its own declared δ implies,
the scatter stops showing up at all.

The clean demonstration holds the geometry **exactly** fixed — the same circle, the same two beds —
and sweeps only the incidence obliquity, which is the *other* attributed term's own variable:

| incidence off the shore normal | 0° | 6.56° | 20° |
|---|---|---|---|
| ramp term, `mean\|θ\|` | 0.710° | 0.641° | 0.111° |
| the same term, **signed** mean | −0.007° | −0.010° | +0.050° |

A term whose value falls by 6.4× as the *other* term's variable moves is not independent of it. At
20° the circle's `mean|θ|` and `|mean θ|` agree to 0.004°: the distribution has stopped straddling
zero and `mean|·|` has gone blind to every zero-mean term inside it.

**(b) The physics does decompose — in the signed mean.** The same four terms, each measured one at a
time on the circle, add:

| term | signed mean `θ_loc` |
|---|---|
| the meter's floor (rotated straight coast) | −0.117° |
| the march meeting curvature | +0.100° |
| the declared `δ = θ_b` | +1.400° |
| the ramp keying | −0.007° |
| **sum** | **+1.375°** |
| **measured on the built bay under its own fan, spiral span** | **+1.420°** |

3 %. So both of the previous subsection's mechanisms are real, the declared `δ` is the term that
dominates the built bay and neither subsection had named it, and the lesson is narrower than "the
attribution was wrong": **attribute in a statistic that adds, then report in whichever one the
reader needs.** A mean of absolute values is a scatter measure wearing a bias measure's clothes.

**What the fix is worth, on the built bay, under the same fan and the same closure.** Note that the
concentric ramp *lowers the angle and raises the transport*, because `Q ∝ H_b^(5/2)` and a different
bed shoals differently — a second reason not to attribute in one statistic alone. **`M`**

| bay, under the fan its own pole implies | mean \|θ_loc\| | `Q` rms over the spiral span |
|---|---|---|
| axis-keyed ramp (the state before this) | 2.801° | 2.650 × 10⁻² |
| concentric ramp about the pole | 2.759° | 3.189 × 10⁻² |
| **normal-offset ramp** | **2.448°** | **2.454 × 10⁻²** |
| the measured floor, unmoved by the keying | 0.202° | 1.780 × 10⁻³ |

The floor is restated because a small number without one is the fourteenth way a measurement lies
with the sign flipped. The bay is **14.7× the floor after the fix and 15.8× before it** — the fix is
real, it is 12 % of the residual, and the residue is now almost entirely the declared `δ`: the
spiral is *built* to hold a constant residual obliquity at every station, so a spiral bay cannot
read zero and only the circle can.
...
## 14 · The six axes, and why their algebra is not in this file

**A deliberate exception to this file's contract, stated so it does not read as an omission.**
Chapter `12`'s section
[Six axes the rest of this chapter is a point on](12-water-physics.md#six-axes-the-rest-of-this-chapter-is-a-point-on)
— ice on the phase axis, the free jet on the Weber axis, water entry on the impulse axis, the
hydraulic jump, thin-film interference, and vortices on the frame axis — carries **its own formulas inline** rather than
...
doctrine chapter. The six axes are the opposite case. Each is one or two closed forms — Beer's law
...
`openchannel.py`, `thinfilm.py` and `vortex.py`, each of which carries its derivation and its ⚠️ notes in the
docstring beside the code that ships. Guarded by
[`validate_phases.py`](../reference-impl/validate_phases.py) (41 rows, `--bugs` catches 8 of 8),
...
⚠️ **What that costs, named rather than hidden.** Those six modules' fitted constants — the
