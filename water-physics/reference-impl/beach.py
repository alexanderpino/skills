...
# ============================================================================
# THE OFFSHORE CLIMATE HAS MODES, AND THE BED IMAGES THEM -- wave 19
# ============================================================================
# THE GAP THIS ANSWERS, in the critic's own words: "the bed produces ONE surf
# zone where bar section J photographs three to four separated lines; the cause
# is bathymetric, since a Dean monotone ramp plus one Exner bar has one
# breakpoint". It is right, and the missing piece is not in the bed at all --
# it is in the BOUNDARY CONDITION.
#
# Waves 1-18 forced this loop with ONE deep-water height at ONE period. A
# breakpoint model driven by one wave has one breakpoint, and no amount of
# morphodynamics can produce a second from it: `sediment_flux` converges where
# `broken_fraction` collapses, and a single train collapses once.
#
# THE SEA IS NOT ONE WAVE, and on an exposed swell coast it is not even one
# distribution. It is at least TWO simultaneous wave systems with independent
# origins:
#
#   * REMOTE SWELL -- generated by a distant storm, arriving long-period and
#     narrow-band regardless of the local wind. That is `H0_SWELL`/`T_SWELL`,
#     the sea state waves 1-18 already declared.
#   * LOCAL WIND SEA -- generated by the wind blowing over the scene right now,
#     short-period and steep. That wind is ALREADY DECLARED: `U10_SCENE`, the
#     same 6 m/s the glitter path's width and the whitecap coverage are read
#     from. `wind_sea_pm` below turns it into a height and a period, so the
#     second partition costs NO new scene declaration.
#
# The two break at different depths, because the breaker depth is H_b/gamma and
# H_b is a function of the offshore height AND period. Each partition therefore
# runs its own convergence, and Exner turns TWO convergences into TWO bars.
#
# WHY THIS IS NOT THE THING WAVE 2 ALREADY KILLED, and the distinction is the
# whole result. Wave 2 tested a RAYLEIGH height distribution -- five equal-
# probability heights of one H_rms at one period -- and measured that it widens
# the bar and lowers its relief without ever splitting it. That is correct and
# it reproduces here (`--bugs` carries it as a control). The reason is that a
# Rayleigh is UNIMODAL and CONTINUOUS: its breakpoints fill a band with no gap
# in it, so the convergences overlap and add to one broad convergence.
#
#     THE BED IMAGES THE MODES OF THE OFFSHORE CLIMATE, NOT ITS WIDTH.
#
# and the criterion for "distinct" is a length, derived in `mode_separation`.
#
# WHAT IS *NOT* THIS MECHANISM, measured this wave and recorded so nobody pays
# for it twice: a TIME-VARYING single train does not do it either. A tide
# (`z_of_t`) translates the breakpoint smoothly and smears the bar -- the crest
# anomaly falls from 1.42 m to 0.59 / 0.45 / 0.26 m at tidal amplitudes of
# 1.0 / 1.5 / 2.0 m -- and a storm/calm sequence translates the ONE bar back and
# forth without ever leaving two. The partitions here are SIMULTANEOUS, which is
# what makes summing their fluxes at every step the right arithmetic rather than
# an average over a history the bed is fast enough to follow.



def evolve_climate(x, h0, climate=None, theta0=THETA0_SWELL, n_steps=N_STEPS,
                   dt=DT_MORPH, **flux_kw):
    """`evolve`, with the offshore boundary condition carrying more than one
    partition. The loop is unchanged; only the flux is a sum.

        q(x) = SUM_p  w_p * q(x; H_p, T_p)

    THE SUM IS AT EVERY STEP AND THAT IS THE PHYSICS, not a convenience. The
    partitions are SIMULTANEOUS -- a swell and a wind sea are both on the water
    at once -- so the sand at x feels both of them within one wave group, and
    the bed integrates their combined transport. Averaging a flux over a
    sequence of sea states the bed is fast enough to follow (a storm, then a
    calm) would be a different and WRONG statement, and `evolve_forced` is
    where that one lives.

    THE CROSS TERMS ARE DROPPED AND IT IS NAMED. The energetics moment of the
    combined near-bed velocity is <|u|^2 u> with u = u_1 + u_2, which is not
    the sum of the two moments: two trains with independent phases leave cross
    terms that vanish only in the mean of the ODD moments and not in the
    even ones. Summing the partition fluxes is the standard first
    approximation and it is the one taken here; what it costs has NOT been
    measured this wave and the row that says so is an OPEN, not a PASS.
    """
    climate = CLIMATE_SCENE if climate is None else climate
    h = np.asarray(h0, float).copy()
    dx = float(x[1] - x[0])
    wsum = float(sum(p[0] for p in climate))
    for _ in range(int(n_steps)):
        q = np.zeros_like(h)
        d_ref = None
        for w, Hp, Tp in climate:
            tr = transform(x, h, Tp, Hp, theta0)
            q += w * sediment_flux(tr, **flux_kw)['q']
            d_ref = tr['d']
        h = exner_step(h, q / wsum, dx, dt, d_ref)
    return h, [transform(x, h, Tp, Hp, theta0) for _, Hp, Tp in climate]


def mode_separation(d_b, x_b, k_dally=K_DALLY):
    """Are two partitions' breakpoints far enough apart to leave two bars?

    DERIVED, from the decay model the transform already marches. A breaking
    wave's energy flux relaxes toward its stable value over the Dally length
    `d/K` -- that is the exponent in `F_{i+1} = F_s + (F_i - F_s) exp(-K dx/d)`
    and nothing else -- so the band over which partition p's onshore flux
    collapses, and therefore the width of the convergence Exner integrates, is
    `d_bp / K`. Two convergences are DISTINCT when their half-widths do not
    overlap:

        |x_b1 - x_b2|  >  (d_b1 + d_b2) / (2*K)

    Every quantity on both sides is an output: `x_b` and `d_b` come out of
    `breaker_state`, and K is the transform's own decay coefficient. Nothing
    about bars, spacing or counts is declared anywhere.

    IT IS A NECESSARY CONDITION AND NOT A SUFFICIENT ONE, which is why the
    suite tests the count as well as the criterion. Two convergences that do
    not overlap can still fail to leave two RIDGES if one partition's undertow
    scours the other's crest -- and that is a statement about relief, which is
    measured, not predicted here.
    """
    d_b = np.asarray(d_b, float)
    x_b = np.asarray(x_b, float)
    gap = float(np.abs(x_b[1] - x_b[0]))
    need = float((d_b[0] + d_b[1]) / (2.0 * k_dally))
    return dict(gap=gap, need=need, ratio=gap / need, distinct=gap > need)
...
def breakpoints(tr, gamma_b=GAMMA_B, d_valid=D_MORPH_MIN):
    """THE ROUND'S MEASURABLE: the x at which H/d crosses the breaker index
    from BELOW, one entry per crossing.

    `d_valid` DROPS CROSSINGS IN WATER THE MODEL REFUSES TO ANSWER FOR, and it
    is not a cosmetic filter -- it is `D_MORPH_MIN`, the same gate `exner_step`
    uses, with the same one-line reason attached: "the bed inside the swash is
    shaped by swash, which is not modelled here, so the loop is not allowed to
    invent an answer for it". Without it this function reports a second
    breakpoint ON THE BARE DEAN RAMP at x = 496 m in 0.31 m of water, where
    `transform`'s own `D_MIN = 0.10 m` depth floor stops the depth falling while
    H keeps arriving, so H/d climbs back through gamma as an artefact of the
    floor. A count that includes it is 2 before this round did anything, which
    would have made the round's headline number a measurement of a clamp.

    `break_lines` above returns INTERVALS and a saturated inner surf zone can
    hand it one interval a hundred metres long -- which reads as "one breaking
    line" when it is a whole band, and reads as "one" whether the profile has
    one bar or three, if the wave never gets back under gamma between them. The
    count that answers the critic's pixel measurement is the number of times the
    wave STARTS breaking, and starting requires the ratio to have been below
    gamma first.

    The crossing is INTERPOLATED between the two cells that straddle it, for
    the reason `breaker_state` interpolates: on a 1 m grid across a bar's flank
    the first cell over the index overshoots gamma by about 3%, and a
    discretisation artefact that size looks exactly like a wrong constant.

    Returns a list of dicts, seaward first: x, d, H, and the ratio at the cell
    after the crossing (which is >= gamma by construction and is reported so a
    reader can see how far past it the grid landed).
    """
    H, d, x = tr['H'], np.maximum(tr['d'], D_MIN), tr['x']
    r = H / d
    out = []
    for i in range(1, r.size):
        if r[i - 1] < gamma_b <= r[i]:
            if d_valid is not None and d[i] < d_valid:
                continue
            f = (gamma_b - r[i - 1]) / (r[i] - r[i - 1])
            out.append(dict(x=float(x[i - 1] + f * (x[i] - x[i - 1])),
                            d=float(d[i - 1] + f * (d[i] - d[i - 1])),
                            H=float(H[i - 1] + f * (H[i] - H[i - 1])),
                            ratio_cell=float(r[i]), i=i))
    return out


def climate_breakpoints(x, h, climate=None, theta0=THETA0_SWELL,
                        gamma_b=GAMMA_B, merge=None):
    """Every distinct position at which ANY partition of the offshore climate
    starts breaking on this bed -- the count a photograph of the water returns.

    THE METRIC HAS TO BE STATED OVER THE SEA THAT IS ACTUALLY THERE, and this
    is the honest form of it. `breakpoints(tr)` counts the crossings of ONE
    train; a frame shows all of them at once, because a swell and a wind sea
    are both on the water and both break. Two partitions breaking 94 m apart
    are two white lines whether or not either train breaks twice by itself, and
    the critic's measurement is of lines.

    `merge` is the distance below which two partitions' breakpoints are counted
    as one line rather than two; it DEFAULTS to the derived one --
    `mode_separation`'s (d1+d2)/(2K) evaluated on the pair -- rather than to a
    round number, so the count and the criterion cannot disagree.

    Returns dict(x=[...], d=[...], per_partition=[n, ...], n=len(x)).
    """
    climate = CLIMATE_SCENE if climate is None else climate
    xs, ds, per = [], [], []
    for _, Hp, Tp in climate:
        tr = transform(x, h, Tp, Hp, theta0)
        bp = breakpoints(tr, gamma_b)
        per.append(len(bp))
        for b in bp:
            xs.append(b['x'])
            ds.append(b['d'])
    ...
    kx, kd = [], []
    for xi, di in zip(xs, ds):
        if kx:
            lim = (merge if merge is not None
                   else (di + kd[-1]) / (2.0 * K_DALLY))
            if xi - kx[-1] <= lim:
                continue
        kx.append(xi)
        kd.append(di)
    return dict(x=kx, d=kd, per_partition=per, n=len(kx), x_all=xs, d_all=ds)
...
# `?` ON BOTH COEFFICIENTS BELOW, and they are the only new numbers this
# section introduces. What is DERIVED is their form: a fully developed sea has
# only two scales, the wind speed U and gravity g, so dimensional analysis
# forces H_s = c_H * U^2/g and T_p = c_T * U/g and there is nothing else it
# could be. The coefficients are Pierson & Moskowitz's (1964) and are RECALLED
# rather than read -- no copy of the paper was available in this container -- so
# they are marked, and the finding is defended the way `K_DALLY` is: by a sweep
# rather than by a citation. `validate_beach.py` runs the wind sea over a factor
# of two in BOTH and reports that the two bars survive wherever
# `mode_separation` says they should, which is a stronger statement than the
# citation would have been.
PM_HS_COEF = 0.0246 * G     # so that H_s = PM_HS_COEF * U^2 / g. The familiar
                            # form is H_s = 0.0246 U^2 with U in m/s and H in m,
                            # which hides a g; it is written with the g visible
                            # because the g is what makes it dimensionless.
PM_WAVE_AGE = 1.14          # c_p / U10 at full development. `?`, recalled.


def wind_sea_pm(u10=U10_SCENE, g=G):
    """The locally generated wind sea, from the wind the scene already declares.

        H_s = PM_HS_COEF * u10^2 / g          (Pierson-Moskowitz, `?`)
        c_p = PM_WAVE_AGE * u10   ->   T_p = 2*pi*c_p/g

    NOT A NEW SEA STATE. `U10_SCENE` is the same wind `beach_optics.U10` gives
    the glitter and the whitecaps, and the suite already fails if the two
    differ. This is a FOURTH readout of it: the glitter's width, the whitecap
    coverage, the swell's directional spread -- and now the second partition of
    the offshore climate.

    At the scene's 6 m/s that is H_s = 0.886 m, T_p = 4.38 s, and it breaks in
    1.18 m of water against the swell's 2.33 m. Both numbers are outputs of
    `transform`; neither is set here.
    """
    u = float(u10)
    return dict(Hs=PM_HS_COEF * u * u / g,
                Tp=2.0 * math.pi * PM_WAVE_AGE * u / g,
                cp=PM_WAVE_AGE * u, u10=u)


def offshore_climate(u10=U10_SCENE, H0=H0_SWELL, T=T_SWELL, w_sea=0.5):
    """The scene's offshore boundary condition as a list of PARTITIONS.

    Returns [(weight, H0, T), ...], weights summing to 1. `w_sea` is the wind
    sea's share of the transport moment, `?` at 0.5 -- "neither system is
    privileged", the same declaration `LAM_U` makes for the two flux terms. The
    suite sweeps it from 0.15 to 0.85 and reports what moves: the bars' CREST
    DEPTHS do not, because each is set by its own partition's H_b/gamma; their
    AMPLITUDES do, because amplitude is paid for in transport.

    ONE PARTITION IS THE OLD SCENE EXACTLY. `offshore_climate(w_sea=0.0)`
    returns a single (1.0, H0_SWELL, T_SWELL), and `evolve_climate` on it is
    arithmetically identical to `evolve` -- the suite carries that as a row, to
    round-off, because a new entry point that quietly changes the old answer is
    how 562 measurements stop being comparable in one move.
    """
    ws = wind_sea_pm(u10)
    if w_sea <= 0.0:
        return [(1.0, float(H0), float(T))]
    if w_sea >= 1.0:
        return [(1.0, ws['Hs'], ws['Tp'])]
    return [(1.0 - float(w_sea), float(H0), float(T)),
            (float(w_sea), ws['Hs'], ws['Tp'])]


CLIMATE_SCENE = tuple(offshore_climate())
...
def evolve_2d(x, y, h0, T, H0, theta0, n_steps, dt, k_every=1, climate=None,
              **flux_kw):
    ...
    WAVE 19. `climate` is the multi-partition offshore boundary condition of
    `evolve_climate`, in plan. It is None BY DEFAULT and that is deliberate for
    the third time in this file, after wave 9's `embay` and wave 16's `stands`:
    every measurement waves 1-18 published was taken on the single-partition
    bed, and a wave that changes the boundary condition under them makes 562
    rows incomparable in one move. With `climate=None` this is byte-for-byte
    wave 18's function, and the suite carries that as a row.

    Each partition gets its OWN dispersion solve, cached on the same `k_every`
    schedule -- the wavenumber is a function of the period as well as the depth,
    so sharing one `k_field` across partitions would silently transport the
    wind sea at the swell's wavelength.
    """
    h = np.asarray(h0, float).copy()
    dx = float(x[1] - x[0])
    dy = float(y[1] - y[0])
    ...
    for s in range(int(n_steps)):
        if kf is None or s % k_every == 0:
            ...
            if climate is not None:
                kfs = [wavenumber(2.0 * math.pi / Tp, dd)
                       for _, _, Tp in parts]
        if climate is None:
            tr2 = transform_2d(x, y, h, T, H0, theta0, k_field=kf)
            fl = sediment_flux_2d(tr2, **flux_kw)
        else:
            # THE SUM IS THE ONLY CHANGE. Same transform, same flux, same
            # Exner; the boundary condition carries more than one partition and
            # the flux is their weighted sum, exactly as in `evolve_climate`.
            tr2 = None
            fl = None
            for (w, Hp, Tp), kfp in zip(parts, kfs):
                trp = transform_2d(x, y, h, Tp, Hp, theta0, k_field=kfp)
                flp = sediment_flux_2d(trp, **flux_kw)
                if fl is None:
                    tr2 = trp
                    fl = dict(qx=w * flp['qx'] / wsum, qy=w * flp['qy'] / wsum)
                else:
                    fl['qx'] += w * flp['qx'] / wsum
                    fl['qy'] += w * flp['qy'] / wsum
