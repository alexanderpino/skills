...
CONTRACT >= 80% of the target bar for <= 20% of the undisciplined cost, scored
         from the log by `efficiency`; baseline round = <first light's tokens>
...
**The efficiency contract.** State it in the block rather than leaving it as a
property of the tooling: this run promises **≥ 80% of the target bar for ≤ 20% of
what the same run costs without the discipline**, and `efficiency` scores it from
the log at every wave boundary. Two things make it real at intake and nowhere
else. First, **the baseline round**: first light is the only unoptimised round
the run will ever have, so record what it cost (`--baseline-round-tokens`) or the
ratio has no denominator for the rest of the run. Second, **the levers**: `init`
prints a preflight naming any that are off — no gates declared, a WIP limit that
funds every lane — because they are configuration decided here, and a contract
found unreachable at wave 9 was unreachable at wave 0. Lower it deliberately
(`--cost-ceiling`) if the run genuinely cannot carry the levers; do not leave it
stated and unmet.
...
python3 scripts/gauntlet.py init --lanes <a,b,c> --dimensions <d1,d2> \
    ...
    --baseline-round-tokens <first light's measured cost>
```
