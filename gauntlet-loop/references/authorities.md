...
pessimism — it is how a fixed budget finds the good option at all. And when
every lane retires early, the unspent share is the run's deliverable: it
returns to the user by default, with a stretch block offered as something they
may buy — never silently spent.
