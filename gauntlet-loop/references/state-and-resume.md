...
├── skips.jsonl      # append-only; rounds deliberately not run, and what each saved
├── spend.jsonl      # append-only; token spend that produced no round
├── state.json       # generated by `gauntlet.py board`; the workbench spec
├── state.js         # the same spec as a script assignment, for file:// viewing
├── report.md        # drafted by `gauntlet.py report` at the end
└── workbench.html   # copied from assets/; generic renderer, never hand-edited
```

`state.json`, `state.js` and `report.md` are all derived from the two ledgers —
if they are ever lost or look wrong, regenerate rather than repair.
