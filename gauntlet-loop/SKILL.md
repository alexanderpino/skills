...
| Token burn | Cost per closed gap climbing wave over wave | Machine gates, one critic call, WIP limit |
...
A round is one builder call plus one critic call (two when split); a wave is that
times the funded lanes, plus one smoother. A 3-lane, 8-wave run is roughly 56
invocations — `init` prints the projection, and a 3-wave extension on one
surviving lane is ~9 calls, not another hundred. Parallel lanes raise the burn
*rate*, not the total. Say so before starting when the projection looks
disproportionate to the artifact, and watch cost per closed gap in `status`: when
it climbs wave over wave, the honest offer is a stop.
