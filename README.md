...
| [`gauntlet-loop`](gauntlet-loop/) | Quality iteration | Run a **smart gauntlet loop** — the same result the full-grind gauntlet would reach, delivered faster and cheaper: an adversarial build-and-judge loop that pushes an inspectable artifact past "good enough" toward a *reachable* target bar: split the goal into ranked, independently judgeable lanes, fund a few per wave under a WIP limit, give each a builder and a *separate* blind critic in fresh context, guard every round with a champion/challenger revert, park lanes the log shows have stopped moving, and iterate until an armed stop condition fires — then, when the stop is the budget, stop and offer a priced extension rather than ending a run that was still improving. Spends judgement only where a decision lands: blind champion/challenger promotions by default, screening-tier verdicts for routine rounds with deciding-tier verdicts required at lifecycle turns, hash-cached machine gates harvested from critic findings as the run learns, diff-scoped repeat inspections, and a softening tripwire that flags severity easing over near-empty diffs. Ships an intake contract with kill criteria, target/stretch bars, per-role model routing and cost accounting, frozen scope with a backlog, named failure modes, stdlib-only state tooling, and a sourced rationale for each rule. Method credited to Matt Shumer. |
...
│   └── scripts/                       # run-state CLI (init/log-round/status/park/board/extend/report)
