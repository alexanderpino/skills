...
| [`straight-answer`](straight-answer/) | Response & scope discipline | Answer exactly what was asked and change exactly what was requested — nothing more. Matches the answer's shape to the question's (a closed question gets `Yes.` as its first word, a lookup gets a path, an "A or B?" gets the pick), filters every qualifier, caveat and alternative through one test — *would the reader do something different if they knew it?* — and bounds the diff to what the request named plus what the repo requires, reporting anything else as a `Noticed:` line instead of fixing it. Carries an explicit floor it never cuts: the answer itself, an action-changing qualifier, whatever failed or is unverified, and a genuinely blocking question. Model-invocable, or forced with `/straight`. |
...
npx skills add github:alexanderpino/skills/straight-answer
```
...
├── straight-answer/                  # unpacked, reviewable skill
│   ├── SKILL.md                       # the two creeps, the shape table, the floor
│   ├── references/                    # before/after pairs, incl. what only looks like creep
│   ├── commands/                      # optional /straight wrapper; the skill provides the command itself
│   └── evals/                         # trigger + behaviour evals
