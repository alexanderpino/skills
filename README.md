...
| [`gauntlet-loop`](gauntlet-loop/) | Quality iteration | Run an adversarial build-and-judge loop that pushes an inspectable artifact past "good enough" toward a *reachable* target bar: split the goal into ranked, independently judgeable lanes, fund a few per wave under a WIP limit, give each a builder and a *separate* blind critic in fresh context, guard every round with a champion/challenger revert, park lanes the log shows have stopped moving, and iterate until an armed stop condition fires — then, when the stop is the budget, stop and offer a priced extension rather than ending a run that was still improving. Ships an intake contract with kill criteria, target/stretch bars, machine gates, per-role model routing and cost accounting, frozen scope with a backlog, named failure modes, stdlib-only state tooling, and a sourced rationale for each rule. Method credited to Matt Shumer. |
