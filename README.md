...
| [`gauntlet-loop`](gauntlet-loop/) | Quality iteration | Run an adversarial build-and-judge loop that pushes an inspectable artifact past "good enough" toward a reference-class bar: split the goal into independently judgeable lanes, give each a builder and a *separate* blind critic in fresh context, guard every round with a champion/challenger revert, and iterate until an armed stop condition fires — then, when the stop is the budget, stop and offer a priced extension in waves rather than ending a run that was still improving. Ships an intake contract, per-dimension bars, named failure modes, and stdlib-only state tooling. Also runs in author mode, rendering a confirmed contract into a portable gauntlet prompt for agents that don't have the skill installed. Method credited to Matt Shumer. |
