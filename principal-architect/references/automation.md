...
- **Freshness**: a `current`/`accepted` **living** doc whose newest
  `last-reviewed`/`updated`/`date` is older than a threshold gets a staleness warning
  (`arch_lint.py --max-age-days`, default 180, `0` disables) — a "living" doc that nobody
  has re-verified in half a year is drifting by default. Re-verify and stamp
  `last-reviewed:` (`conventions.md` §2). ADRs/RFCs are exempt: decision records are
  immutable history, superseded rather than re-reviewed.
