...
| Forward design method | **ADD 3.0** — Attribute-Driven Design (Cervantes & Kazman, *Designing Software Architectures: A Practical Approach*; SEI) — the design-side sibling of ATAM and the quality-scenario form | Driver-led iterations for greenfield/To-Be structure: iteration goal from the utility tree, design concepts from `structure.md`, each round closes with C4 views + ADRs and an ATAM-lite check. Method in `methods.md` §13. |
...
| Security / threat modelling | **STRIDE** (Microsoft) + the **OWASP family**: **Top 10:2025** (web), **API Security Top 10:2023** (published APIs/events), **Top 10 for LLM Applications:2025** (LLM components), **ASVS 5.0** (verification levels L1–L3 as the requirements-side anchor), **SAMM** (org maturity, enterprise altitude) · SBOM: **SPDX** (ISO/IEC 5962:2021) / **CycloneDX** (ECMA-424) | Mandatory threat-model section in HLD §8 and SAD §8 — pick the OWASP lens per surface; target ASVS level recorded as a `C.xx`; SBOM generated in CI as the A03 supply-chain evidence (`automation.md` §6). |
...
| Cloud architecture | **ISO/IEC 22123** (cloud vocabulary, concepts & reference architecture — successor to 17788/17789; defines IaaS/PaaS/SaaS) · **ISO/IEC 19086** (cloud SLA framework; -4 covers security/privacy SLA components) · **ISO/IEC 27017/27018** + **CSA STAR/CCM** (cloud security controls & PII protection — the certifications providers hold) · vendor **Well-Architected** frameworks (AWS/Azure/Google) | Cloud is a deployment substrate, not a fourth altitude — no separate doc type. Service-category choice in SAD §6 (22123 terms; lock-in/egress in the ADR); shared responsibility split in HLD/SAD §8 (27017); provider SLAs (19086) feed the solution SLOs. Well-Architected pillars re-project artifacts that already exist: operational excellence/reliability → `operability.md` + `Q.xx`, security → §8 threat model, performance → `Q.xx`, cost/sustainability → §9 FinOps — present them in the org's pillar format (house style), don't write a parallel review. |
...
| Operability / reliability targets | **ISO/IEC 25010:2023** reliability + **ISO/IEC 25023** (SQuaRE measures) · **ISO/IEC 20000-1** (service level management — the SLA side) · **ISO 22301** / **ISO/IEC 27031** (business continuity — RTO/RPO) · **Google SRE** (Beyer et al. 2016/2018) — SLI/SLO/error-budget mechanics · **OpenTelemetry** | Operational `Q.xx` response measures translate to SLOs over SLIs; RTO/RPO trace to the BCMS where one exists; SLO table in HLD §7 / SAD §10. Reliability-driven topology is an ADR. `operability.md`. |
| Data architecture | **ISO/IEC 25012** (SQuaRE **data** quality model) · **ISO 8000** (data-quality management) · **ISO/IEC 9075** SQL / **ISO/IEC 39075:2024** GQL (query-language portability) · **DAMA-DMBOK 2** (orientation) · strategic-DDD data ownership · **CAP** (Brewer; Gilbert & Lynch 2002) / **PACELC** (Abadi 2012) · **expand–contract** (Fowler) | Ownership & boundaries, storage selection keyed to `Q.xx`, data-quality drivers written against 25012 characteristics, schema evolution without breakage; slice-scoped ERDs generated from the schema. `data-architecture.md`. |
| Business processes | **BPMN 2.0** (OMG; **ISO/IEC 19510:2013**) | Enterprise/solution process views when a change alters a business process; mermaid lane-flowchart approximation by default, the org's real `.bpmn` XML as source of truth where one is authored/executed. `mermaid-guide.md`. |
| Documentation prose & tone | **SEI seven rules** (*Documenting Software Architectures*, Clements et al., 2nd ed.) · **Nygård** (value-neutral context, committed "We will…" decisions) · **Hohpe**, *The Software Architect Elevator* (penthouse↔engine-room altitude of language) · **Brown** (C4 labeling discipline) · **Fowler** (semantic diffusion; example-led bliki style) | Reader-first, one home per fact, verdicts not surveys, numbers over adjectives, defined terms reused exactly; the any-system test against generated-sounding prose. House tone overrides the register, never the precision. `writing-style.md`. |
| AI-bearing systems | **EU AI Act** (Reg. (EU) 2024/1689, risk tiers) · **ISO/IEC 22989:2022** (concepts & terminology) · **ISO/IEC 23894:2023** (AI risk management) · **ISO/IEC 42001:2023** (AI management systems) · **ISO/IEC 42005:2025** (AI system impact assessment — the DPIA counterpart) · **NIST AI RMF 1.0** · **model cards** (Mitchell et al., FAT* 2019) | Risk tier as a `C.xx` constraint; model quality targets as ordinary `Q.xx` scenarios (eval suite = means of verification); obligations mapped to existing artifacts in `privacy.md`. |
