...
| Operability / reliability targets | **Google SRE** (Beyer et al., *Site Reliability Engineering* 2016; *The Site Reliability Workbook* 2018) — SLI/SLO/error budget · **OpenTelemetry** (observability instrumentation) | Operational `Q.xx` response measures translate to SLOs over SLIs; RTO/RPO as recoverability measures; SLO table in HLD §7 / SAD §10. Reliability-driven topology is an ADR. `operability.md`. |
| Data architecture | **DAMA-DMBOK 2** (orientation) · strategic-DDD data ownership (one owning context per dataset) · **CAP** (Brewer; Gilbert & Lynch 2002) / **PACELC** (Abadi 2012) · **expand–contract / parallel change** (Fowler) | Ownership & boundaries, storage selection keyed to `Q.xx`, schema evolution without breakage; slice-scoped ERDs generated from the schema. `data-architecture.md`. |
...
| AI-bearing systems | **EU AI Act** (Reg. (EU) 2024/1689, risk tiers) · **ISO/IEC 42001:2023** (AI management systems) · **NIST AI RMF 1.0** · **model cards** (Mitchell et al., FAT* 2019) | Risk tier as a `C.xx` constraint; model quality targets as ordinary `Q.xx` scenarios (eval suite = means of verification); obligations mapped to existing artifacts in `privacy.md`. |
