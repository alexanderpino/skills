...
5. The committed worked example under `examples/brownfield-api-keys/` (a full doc set
   ...
   regressions.
6. The **staleness check** fires correctly in both directions: a `current` doc
   last updated in 2020 draws a `stale: last reviewed/updated …` warning by
...
committed dates are pinned), so no assertion decays as wall-clock time passes.
