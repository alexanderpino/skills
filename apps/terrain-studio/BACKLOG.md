...
| W15 | **S4.7's displaced water is unconfirmed VISUALLY** | The maths is gated (5/5 armed, 1e-9 against a double oracle), both passes provably share one generated source, and the shader compiles with no page errors. What is NOT done is seeing it: four attempts failed to get liquid water on screen. A hand-built perlin->water(sea)->output graph leaves `scene.water` false; `showcaseGraph()` sets it true but renders entirely snow and ice at -4 degC, and setting `terrainDef.seaTemp` does not move the HUD because the temperature comes from the graph's own chain. **What is actually needed** is a fixture graph with liquid water in frame — probably a temperature node authored warm, not a datum tweak — and then S4.10's pixel comparison: render with `waterLook.strength` 0 and 1 and require the images to differ. That comparison is the real gate; a screenshot is only how a human checks it. |
| W16 | **S4.7 remainder** | Displacement does not fade below one projected pixel, and the water mesh bounds are not inflated by the declared amplitude (`presetBounds` reports 2.36 m vertical / 1.52 m horizontal and nothing consumes it). Both are named in the story's acceptance and neither is gated. |
...
1. **A plugin re-derives the document's own grid geometry by hand.** Width from `Math.round(Math.sqrt(N))`, cell size from `terrainDef.scale/n`, hex row pitch from a fresh `Math.sqrt(3)/2`. Correct on square, wrong or divergent on hex. `d_windmodify:42` was a **live defect** — fixed by `widthForLength()`. Still open: `fracture.js:115-116`, `surface.js:130,140`, `mathnode.js:106`, `craterfield.js:6-20`. *Fix:* one import list plus a shared sample→authoring-coordinate helper, then a scan that fails the build on `terrainDef.scale/` or `Math.sqrt(3)` arithmetic under `src/plugins/`.
2. **The default world extent, `5000`, frozen into plugins** as a slider ceiling or a stored-value clamp, not derived from the live document. On a 20 km world these saturate at a quarter of the map; on a 500 m world they exceed it. Worst in `gen/`, where `bounded()` clamps the **stored** value and the author gets a small island with no diagnostic. ~16 literals across `gen/`, `filt/`, `surface/`. *Fix:* one bound helper reading `getWorldDomain().extentM`.
3. **A default written in both the plugin schema and the kernel signature.** `hydraulic`'s copies have **already diverged** — capacity 6 vs 4, erode .35 vs .3, deposit .28 vs .3, droplets 18000 vs 15000. `surface` writes all 21 params' default *and* range twice and its evaluator re-clamps to its own copy. `d_temperature` writes seaTemp/lapseRate four times. *Fix:* one exported default table per kernel.
4. **A slider formatter restating the kernel's coefficients**, so the label can silently lie — and labels are not digest-covered. `canyon.js:19` restates `legacy.js:1121`; `erosion2.js:17` restates `legacy.js:3046`.
5. **A composite node re-implementing shipping analysis nodes inline** instead of taking a Driver port, exposing their tuning as its own params — and the ranges have already diverged, so one physical quantity has two authorings with different admissible sets. `surface` (curvature 0-4 vs `d_curvature`'s 0.1-4, wear 0-4 vs 0.2-3), `satmap`, `d_texture`, `slopemask`.
6. **A world distance authored in lattice cells** in a registry whose standard is metres-through-`cellSizeM()`. `blurField` is called by Blur with a *cell* radius and by `sharpenField` with `radiusM/cellSizeM()` — one kernel serving two incompatible scales, and with Res Lock off the cell form becomes resolution-dependent while every `radiusM` node is immune.
7. **A fact about a node kept as a hand-written list elsewhere**, which a new node silently misses. All three are currently incomplete and two are *armed gates under-reporting*: the terrain-definition invalidation list (`legacy.js:4192-4194`, hydraulic and erosion2 absent), `DERIVED_PRODUCERS` (`doctrine.js:33-36`, `d_texture` absent though `legalOrder` is armed), and the blend-mode vocabulary hand-subsetted in three plugins. *Fix:* move each fact onto the plugin definition so the lists derive themselves.
8. **A world climate or physics constant re-authored per node.** `terrainDef` already carries latitude, north, seaTemp, lapseRate, solarElevation, windDirection, windSpeed; `snow` restates windDirection (four copies in the tree) and offers windStrength in an incomparable unit. Gravity (`crater`, `craterfield`) has no home in the document at all.
9. **Vertical quantities as a bare fraction of `terrainDef.height` with no datum**, so re-scaling relief silently moves them in metres — the vertical twin of the pixel-radius resolution dependence these plugins otherwise avoid. `water.js:16,19`, `heightmask.js:13`, `surface` contours.
10. **A node's real authored state living outside its schema**, created by a type-name special case in the graph core, so `params: []` is a false statement about the node. `drawmask` strokes and `colormixer` layers — the same shape that already required guarded fixture lines in the digest.

**Dismissed explicitly**, with reasons, rather than padding the list: `crater.js:65`'s `3200*(9.81/gravity)` is one physical formula (Earth's simple/complex transition diameter and the gravity it was measured at), not a duplicated constant; `gate`/`softclip`/`threshold`'s ±100000 is the house idiom for unbounded numeric entry; `clampn`'s [0,1] range is its own ordinary choice under ADR-005.
