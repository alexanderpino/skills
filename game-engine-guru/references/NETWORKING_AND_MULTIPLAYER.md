...
**Re-simulation budget.** At 60Hz, budget 8 frames of rollback = 133ms of network jitter tolerance. Each frame of re-sim must complete in `(16.67ms - current_frame_work) / rollback_frames`. A 2ms sim step gets you 8 rollback frames in a single 16ms frame. Keep the sim lean.
...
**Constraints.** Simulation must be fully deterministic. See `PHYSICS_MATH_AND_SIMULATION.md` §9 — fixed timestep, FMA-off, platform-agnostic transcendentals, no `std::mt19937`.
...
**Quantization.** Position: 0.1mm (0.0001m) precision → 20-24 bits per axis. Rotation: smallest-three quat, 16 bits per component → 50 bits total. Velocity: coarser, 0.01 m/s → 16 bits per axis. Cuts bandwidth 4-6x over raw floats.
