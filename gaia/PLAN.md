...
| **Provenance in practice** | 6 | **6** — **unmoved, and this is the honest limit the plan predicted.** The artefact cache is still 6 located of 119 with 112 UNFETCHED, and **neither branch of the gate has been taken**: no mechanism named, no admission written. `requote.py` — the instrument aimed at the corpus's self-declared most damaging defect — has still never run against the corpus, because there is nothing for it to read | X25/X45 fixed **and** the artefact cache either reaches ≥ 60% or is declared unbounded | Phase 3, Phase 6 — **the honest limit** |
