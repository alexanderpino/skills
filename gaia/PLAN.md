...
| 2 | Every block a reader would paste is correct; every other block runs or is gone | ❌ **NOT MET.** Second half done, first half never started. **104 fenced blocks across 35 of 40 documents against 107 register rows that name a block, and 0 documents holding a fence with no row**; the `termination` column is enforced on 127/127 rows, 1 `unknown`. But **nothing executes a `## Use this` block**: `grep -c rigs .github/workflows/gaia.yml` → 0, and there are **16** such blocks (not 8) across 12 documents, of which the register names 4. The plan's own open choice at `:97` — real Python in CI, or mark them illustrative — is still open, and "0 fail" has no population until it is taken. ⚠️ 104 fences against 107 rows is **two counts, not a matching**: `check.py:2431` says so itself | the 8 blocks inside `## Use this` pass their numeric gates; every fenced block has a register row with a **termination** column; 0 fail |
