...
|---|---|---|---|
| 1 | The six reproduced guard holes are closed | ✅ **MET 2026-09-15.** All six carry a red-and-green `bites` pair, and the job is 41 mutations / 6 decoys / exit 0 run locally. ⚠️ It was **not met** nine hours earlier and an independent verifier said so: five stamp rows were red for an unrelated reason (the fixture's fifth stamp made `SKILL.md` disagree with the tree — fixed in `1b3c064`), and `--list` named none of T3, T4, T8 although all three had been `OPEN` rows since 2026-09-07 (`7921de5`). A CI assertion now checks both ends of that last clause | each has a `bites` row that goes red **and** goes green with its guard stubbed; the three surfaces the guard cannot see (T3, T4, T8) are `OPEN` rows in `guard-proofs.tsv` and named in `--list` |
| 2 | Every block a reader would paste is correct; every other block runs or is gone | ✅ **POPULATION DECIDED AND GATING UNDER WAY — 2026-09-15.** The census this was being assessed on, *16 fenced blocks inside `## Use this` across 12 documents*, **inherited `check.py`'s own blind spot**: 28 of the 40 documents put no fence inside that section at all, so for **70% of the corpus** the block a reader pastes was invisible to the criterion. The decisive pair: `shallow-water.md:133` and `simulation-time-budget.md:77` are the **same CFL formula**, one triaged four ways and the other not triaged at all, solely because a `##` fell between them. **The population is the first fenced block at or after each document's `## Use this` heading — 40 documents, one block each.** A 33-agent triage fleet returned **0 of 16 illustrative**: all gateable or gateable-in-part. **Rigs that assert: 14 of 40** documents, against **0 of 40** when the population was decided this morning. **That is a count of rigs, and it is NOT a count of verified blocks.** Six of the fourteen were then handed to an independent hostile agent, one per rig, each told to hunt four named failure shapes: **all six came back HOLED**, and every builder had already reported finding and fixing its own defects by mutation. The lead's own gate, written and mutation-tested the same hour, was among them — its band was read off the digit count of the string it asserts, so `±0.7 m`, the same claim one digit coarser and 12% off the law, bought a 10× wider gate and went green. All six were then repaired, every named hole closed against the mutation that found it, and **re-attacked by a fresh hostile agent apiece: all six HOLED again**, on 1,465 mutations against round 1's 15–33. Twelve attacks, twelve verdicts. ⚠️ The second round is NOT evidence the repairs failed — the re-attackers re-ran the claimed closures first and confirmed them (*"none of the 16 claimed fires is fake"*); it is evidence that **33 mutations is not a test**. Attack DEPTH is the variable nobody was measuring, and the two rounds are not comparable. ⚠️ Round 2 also found **five false claims in two repairers' own reports**, including one pricing a decline at `sd/mean = 0.43` when its own rig prints 2.15 on that line — a report misdescribing its rig is a defect one level above a holed gate, because the gate can be re-run and the report is what a reader trusts. So the number to report is not 14, not 10 and not 6: it is **0 of 40 blocks independently verified**, and the thing that closes this criterion is not another repair round but a stated mutation budget per gate, met and recorded. All 15 asserting rigs run in CI as hard gates, discovered not typed, and a `guard`-job step fires 8 page mutations across 3 of them, each required to name the figure moved and then to recover. ⚠️ **26 of 40 remain ungated** and the number to report is 14/40, never a pass rate. ⚠️ Building the gates found **13 defects in the pages themselves**, every one filed `pending` and none fixed by the rig that found it: `CAU-PUREWATER-437`, `CAU-B-TWICE`, `CAU-CITES-OVERCLAIM`, `CLD-HEX-6084`, `CLD-HALVING-398`, `HR-APRON-2L`, `SEAM-OPS-306`, `SEAM-221-TRUNC`, `SEAM-INT32-DOUBLED`, `STRAT-DTMAX-5000`, `STRAT-640-638`, `STRAT-25-200`, `SSS-RAMP-ULP`. Four rigs also found defects **in themselves**, by mutation and never by reading: a caustics gate that read `!= 1.0` against a literal in the file written to end typed-in expectations; a seamless gate whose sample grid landed on integer lattice points, so an entire lacunarity section was measuring a field of zeros and printing PASS; a clastic separation measurement that reused the sampler's own scan width, so narrowing the page narrowed the measurement with it; and a raymarching memory identity where `n` fed both sides | every gate has survived an INDEPENDENT hostile pass, not only its author's own mutations — the count that closes this criterion is attacked-and-holding, never rigs-written; `rigs-asserting N/M` stays a reported metric and is not evidence for this row; every page defect a gate finds is a `corrections.tsv` row before the gate lands; a block that genuinely carries no numeric claim is declared illustrative **on the page** with its reason |
| 3 | The two ends never contradict the body | ✅ **MET, qualified.** 9/9 inversions carry a `corrections.tsv` row recording both ends read (X7, X8, X12, X17, X26, X29, X38, X52, X56); re-derived at HEAD, no withdrawn prescription survives at either end in any of the nine. 4/4 `check_repudiation` candidates are triaged, each recorded a false positive with its reason. ⚠️ **Three of the nine rows read `verifier = NONE`** and say so in the row itself (`:150-152`, X12/X29/X56) — declared, not verified, which PLAN rule 1 does not satisfy | each of the 9 has a `corrections.tsv` row showing both ends were read; `check_repudiation` candidates are all triaged, each false positive recorded with its reason |
| 4 | The engine verdict's three blockers are closed | ✅ **MET. 3/3.** The atmosphere contract carries both: *scene-referred linear radiance in `W·m⁻²·sr⁻¹`* (`atmosphere-and-aerial-perspective.md:374`) and the scale *a white Lambertian surface under zenith TOA sun reads `E_sun/π`* (`:380`), with white point, pre-exposure and an `out-of-scope` row for the display transform. The tick policy is stated once and pointed at, sourced `[fiedler_timestep]`, clamped at `N_max·dt` (`simulation-time-budget.md:200-202`); `N_max` appears in no other document. `L_scatter` carries Mobley's single-scattering form with `mobley1994` `F`, obtained and read (`water-optics.md:161-171`), and every printed constant re-derives from its stated inputs | the atmosphere contract carries a unit and a scale; the tick policy is stated; `L_scatter` has a sourced form |
| 5 | The tool verdict is either turned or declared | ✅ **MET on its first branch, and the branch is narrower than "turned".** `resolution-independence.md` is written — 449 lines against the 450 cap, 14 sourced citations, a parameter census, per-family scaling exponents with two verdicts reading **not available**, and an acceptance test — and 8 documents cite it. `progressive-preview` and `edit-history` are `planned` rows at `coverage.md:107-108`, each opening with its question. ⚠️ The verdict was *"NO, not without three documents"*: **one was written and two were demoted to declared gaps.** That satisfies the clause as the plan wrote it and should not be read as the tool half being covered | `resolution-independence` written; `progressive-preview` and `edit-history` are coverage rows with questions; or `SKILL.md` says the tool half is a declared gap |
| 6 | A reader can tell a document's budget regime from the page | ✅ **MET. 40/40** (denominator grew from 37), machine-enforced in **both** directions by `check_budget_agreement` — a regime on the page must be a tag and a tag must be named on the page — with a `bites` row per direction, each proved red-then-green by mutation. The vocabulary is closed to three spellings; `runtime` appeared in 4 tag lines when the plan was written and appears in 0 now. ⚠️ 9 of 40 documents carry **two** budget tags; under a literal *exactly one* reading the number is 31/40. The plan's own *Now* cell counted two-tag documents as tagged (26+5 = 31 of 37 at `79e4133`), so this reading is the plan's own | 37/37 carry a `**Tier:` line that agrees with one canonical budget tag, checked |
...
|---|---|---|---|
| 0 | ✅ 2026-09-07 | `3a4541c` (guards), `4c793eb` (the three `OPEN` rows, and the row that had to wait for them) ⚠️ this cell cited `0334ce1` until 2026-09-15; that commit adds 103 rows to `corrections.tsv` and touches `guard-proofs.tsv` not at all — found by an independent verifier attributing the other five rows | agent-verify-phase0 — ran the `bites` job under `bash -e`: 33 red rows and 4 green, exit 0 in 37 s; every two-pass row red for its **named** rule and green with only that rule stubbed; all 29 stub anchors unique and failing when drifted. Found and closed one hole it created: a `verified:` stamp on apparatus digested the empty string, so one stamp was valid on all nine files |
| 1 ∥ 2 | ⚠️ **partial** — criterion 2's second half closed 2026-09-08; its first half is unstarted, and criterion 3 stands at 9 rows of 9 with **3 of them carrying no verifier** | `7102246` (termination column), `87de03e` (the 48 missing block rows — documents with a fence and no row, 8 → 0), `f6f54d4` (X12, X29, X56, the three inversions that had never been in the register, plus the four repudiation triage rows) | each per-document sitting ran implementer + independent verifier and the signatures are in `corrections.tsv` (`agent-verify-raymarch`, `-m2m`, `-culling`, `-atmosphere`, `-tectonic`, `-timebudget`). Verification wave A (`f38daa0`) then gave all 13 *audit repairs applied* rows a hand told to disprove them: 6 confirmed, 7 disputed, 5 upheld, **2 disputes rejected** by the adjudication stage — 29%, on this project's measured base rate. Its **completeness critic** found two one-end corrections nobody had been pointed at, including `wave-models.md:12` carrying Lamb's decay modulus dimensionally inverted, wrong by 4.01×10³ at λ = 5 cm. ⚠️ `corrections.tsv:150-152` each read *verifier = NONE*, and say so in the row |
| 3 | ✅ 2026-09-07 | `4eca13a` (`gordon1989`, `mobley1994` entered), `8624598` (C1's ratio and F4's `L_scatter` land in `water-optics.md`), `0334ce1` (the rows) | `523e823` re-read both new entries and every document resting on them, repairing `shader-craft` and `wave-models` in the same pass. The B-rows carry their own second hand (`corrections.tsv:131-134`): *DEFECT CONFIRMED and fixed*, *verifier's REPLACEMENT TEXT taken*, *REPRODUCED, no change needed — both markers seen*. F8's verifier struck two overclaims from the row's own derivation cell with no document edit following, because the page was right both times. ⚠️ `corrections.tsv:141` (B8, the `a` versus `a + b_b` denominator against Mobley) is still `OPEN` |
| 4 | ✅ 2026-09-10 | `8624598` + `b63b452` (criterion 4), `4eca13a` + `523e823` + `ac9281b` (criterion 5), `0958004` (criterion 6, the last to hold) | `agent-verify-atmosphere` confirmed the LUT axes, Table 2 and `L = E_sun/π` from source, and found the X8 inversion still live. `agent-verify-timebudget` built its own exact-rational scheduler and **refuted three printed numbers**, including the equal-counts threshold — 20 fps, not 15. Criterion 6 was refused twice before it was granted: `2a0cb6f` recorded 37/39 and **NOT met** rather than rounding, `8e547fd` 38/39, and `0958004` paid for `river-networks.md`'s Tier block by subtraction to reach 39/39. ⚠️ Criterion 4's F3 row (`corrections.tsv:21`) still reads `commit=pending` |
| 5 | ⚠️ **partial** — one of four items. `check_crossrefs` shipped and is enforced; the handoff generalisation, the typed rows and the split are not | `6a2d207` (shipped as `check_paths`; every `document.md` reference in the corpus resolves, and the live dangling `shallow-water.md:212` is gone) | `6a2d207` ran its mutations before claiming them and its red-and-green pairs are in `guard-proofs.tsv`. ⚠️ **Nothing else here was independently verified, because three of the four are unstarted**: the prose `## The handoff` table exists only at `water-optics.md:393` and never reached `driver-fields`, `flow-routing`, `wave-models` or `simulation-time-budget` — so `water-optics.md` is not placed and the skill seam is untested; there are no `resource:`/`consumes:` rows and no `references/quantities.md` (explicitly the maintainer's call); and `wave-spectra` is still a `planned` row at `coverage.md:78` with no Pierson–Moskowitz, JONSWAP or Hasselmann entry to land it on |
| 6 | ⚠️ **partial** — stamps landed, 4 of 40, and only one of them is among the six this phase names; the artefact-cache gate and the `approximation` gate are both open | `7b392bf` + `3210ab9` (the four sign-off conditions), `b6d5f54` (the stamps), `e84e0a1` (re-pointing the mutations the stamping broke) | signed 2026-09-14 by `human:alexander.pino` — four one-page briefs, all four signed **conditionally**, and every condition landed before the stamp did. Digests were generated by `check.py --digest`, never hand-computed, and each asserted ≠ the empty-body digest that once certified nine apparatus files at once. The guard went red first and caught `node-graph-runtime` at 451 lines. Verification wave B (`13a8743`) gave every Phase D row a hand told to disprove it — 8 confirmed, 5 disputed, all 5 upheld, **three against the lead's own work**. A five-lens rating panel then found `μ_d = 0.86` labelled a uniform sky when it is the CIE standard overcast (`72b6e9d`), a rig checking its own remembered numbers (`7579549`), both self-describing files wrong about the corpus (`4d3daeb`), and eleven citations to a file that had never been committed (`1a5f093`). ⚠️ **Open:** the artefact cache is unmoved at 6 located of 119 and neither branch of its gate has been taken; and `approximation` was raised 7/39 → 39/40 while `approximation_coverage()` still asks only for a number on each half, never a `P` locator naming hardware and resolution or an execution-register row — the count moved before the rows were re-graded, which is what ground rule 4 forbids |
...
| **`gpu-driven-culling.md`** | — | **X7** the footprint-mip consequence un-inverted; **G6** the pass topology as a shape, not a slogan; G7 `NonUniformResourceIndex`, the MSAA/conservative-raster caveat; X21 prefix-sum order; the cross-doc rule clause (⚠️ this row called it **X12**, which is `caustics.md:152`; the audit has no ID for the carve-out — the edit is correct and was kept, but it was unmandated) | agrees with bullet 2 at :103 |
...
| Change | Gate |
|---|---|
| **The artefact cache.** `STATE.md:105-110` names why it is 6 of 119: bot challenges, an expired certificate — an *access* problem. Either name the mechanism (a maintainer-side fetch with a browser, committed as a private cache; institutional access; author copies) with a target of `≥ 60%` located and every ALTERED resolved — or **state it as unbounded and outside this plan**. The v1 target had no path to its number | ✅ **2026-09-15 — the ADMISSION branch is taken, and this is the gate closed, not the gate dodged.** It is **unbounded and outside this plan**, for a reason that is stated rather than implied: the missing artefacts are behind **eight named publisher paywalls** — AGU, AIP, Elsevier, GeoScienceWorld, IEEE, Optica, Taylor & Francis, Wiley — reached from a container with no institutional subscription and no browser session, and **27 distinct bibliography ids across the corpus already say so in their own locators**. No agent-side mechanism reaches `≥ 60%` from here; the `60` was a number with no path to it. `requote.py` consequently exits 0 having asserted **nothing** and says so in its own output (`--require-cache` exits 2 for CI), so a green run of it is not evidence and must never be quoted as any. ⚠️ **This is reversible by the maintainer and only by the maintainer**: institutional access, a browser-side fetch committed as a private cache, or author copies would each take the other branch. Until then the corpus's most damaging recurring defect — a quotation that was cut, or invented — is caught by a human reading the source, and by nothing else |
...
| **The 8 recommendation blocks** (was: "executable content") | 3 | **4** — the three the audit called broken were repaired under a sitting's verifier, and that is all. There is **no numeric gate**: nothing in CI executes a `## Use this` block, and there are **16** of them across 12 documents, not 8, of which the register names 4. Criterion 2's open half | all correct under numeric gates | Phase 1 sittings |
| **The oracle** (46 body blocks) | — | **8** — **104 fenced blocks across 35 of 40 documents, 0 documents holding a fence with no row**, and `termination` enforced on 127/127 rows with 1 `unknown`. Not 9, for the reason `check.py:2431` states itself: 104 fences against 107 block-naming rows is **two counts, not a matching**, so it is a floor on the gap and never its size | every fenced block registered with a termination column; 0 fail | Phase 1 enumeration |
| **Recommendations and failure tables** | 5 | **9** — 9/9 inversions carry a both-ends row and none survives re-derivation at HEAD; 40/40 `**Tier:` lines, machine-checked in both directions. ⚠️ 3 of the 9 rows read `verifier = NONE` and say so | 9 inversions closed with both-ends rows; 37/37 `**Tier:` | Phases 2, 4 |
| Provenance as a system | 9 | **9** — holds, and gained a channel: a source declared NOT OPENED in the front matter must now say so in the body, above the first bare citation, in all 11 documents that have one | hold | Phase 0 |
| **Provenance in practice** | 6 | **6** — **unmoved, and this is the honest limit the plan predicted.** The artefact cache is still 6 located of 119 with 112 UNFETCHED. ⚠️ This cell said *neither branch of the gate has been taken* in the very commit (`0637269`) that took the admission branch 300 lines below — a commit contradicting its own message, in the file no guard reads. The gate IS closed, on the admission branch; the DIMENSION does not move, because admitting the limit is not reaching 60%. `requote.py` — the instrument aimed at the corpus's self-declared most damaging defect — has still never run against the corpus, because there is nothing for it to read | X25/X45 fixed **and** the artefact cache either reaches ≥ 60% or is declared unbounded | Phase 3, Phase 6 — **the honest limit** |
| **Guard engineering** | 5 | **9** — all six holes carry a red-and-green pair; the suite is **41 mutations / 6 decoys / exit 0**, every mutation red for its named rule and green with only that rule stubbed. `--list` names T3, T4 and T8, with a CI assertion on both ends of that clause. ⚠️ Still 0 rigs in CI | six holes, red-and-green pairs | Phase 0 |
| Coverage and routing | 8 | **9** — all three: `planned` rows carry their questions, the budget vocabulary is migrated and enforced 40/40 both ways, and `dated-crossover` shipped with 11 fixtures and a CI mutation. It reports **1 of 26** against a doctrine bullet that hand-counted three, which is the metric working, not failing | rows with questions; budget migrated; `dated-crossover` | Phase 4 |
| **Audit state** | 5 | **7** — **4 of 40**, signed `human:alexander.pino` 2026-09-14 against four conditional briefs whose conditions all landed first, and proved to bite in scope and stay silent outside it. Not 9: four is not six, only one of the four is among the six this phase names, and each stamp covers `## Use this` plus the failure table only — **8–19% of its document** | six documents carry `verified:` by a named human | Phase 6 — **no agent can do it** |
| **Usability** | 7 | **9** — F1, F3 and F4 verified closed clause by clause; `resolution-independence.md` written at 449 lines and cited by 8 documents; `shader-craft.md` written and cited by 8 of the 11 rendering documents. ⚠️ The tool verdict wanted **three** documents: one was written and two are declared gaps with questions, which satisfies criterion 5 and does not make the tool half covered | F1/F3/F4/F9 closed; `resolution-independence` written; `shader-craft` written | Phases 3, 4 |
