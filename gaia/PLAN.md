...
| 2 | Every block a reader would paste is correct; every other block runs or is gone | ❌ **NOT MET, and the POPULATION IS NOW DECIDED — 2026-09-15.** The census this was being assessed on, *16 fenced blocks inside `## Use this` across 12 documents*, **inherited `check.py`'s own blind spot**: 28 of the 40 documents put no fence inside that section at all, so for **70% of the corpus** the block a reader pastes is the first fence *after* the heading and was invisible to the criterion. The decisive pair: `shallow-water.md:133` and `simulation-time-budget.md:77` are the **same CFL formula**, and one was triaged four ways while the other was not triaged at all, solely because a `##` fell between them. ✅ **The population is therefore the first fenced block at or after each document's `## Use this` heading — 40 documents, one block each, every document represented.** A 33-agent triage fleet returned **0 of 16 illustrative**: all are gateable or gateable-in-part, which overturns the earlier reading that four carried no numeric claim. ⚠️ Three things must land BEFORE any gate, and two were found by the fleet's completeness critic: `heightfield-raymarching.md`'s termination argument was refuted by its own page four lines later (fixed); the `rigs` job hardcoded `rigs_asserting == 4`, so the first gate landed would have failed the build on a constant (fixed, the list is discovered now); and the register names 4 of the 16, one of which cites a rig that never existed (marked). Until gates exist the number to report is **not a pass rate but “population decided, 0 of 40 gated”** | the 8 blocks inside `## Use this` pass their numeric gates; every fenced block has a register row with a **termination** column; 0 fail |
