...
sources:
  ...
  - { id: lamb_damping, tier: F, locator: "6th ed. 1932, ch. XI Viscosity, Art. 348, pp. 623-624 — eq. 7 da/dt = -2*nu*k^2*a, eq. 8 its exponential decay, eq. 9 the decay modulus tau = 1/(2*nu*k^2) = lambda^2/(8*pi^2*nu). Keep the denominator parentheses: written flat, 1/2*nu*k^2 reads as (1/2)*nu*k^2, which is a RATE and not a time -- inverted, and off by a factor of ~4e3 at lambda = 5 cm. Corrected 2026-09-14; water-closed-vs-open.md carried the parenthesised form already and this end had not moved" }
