...
- **lindsay2016** `P` [not-opened] — Lindsay, J.B. (2016). *Efficient hybrid breaching-filling sink removal methods for flow path enforcement in digital elevation models.* Hydrological Processes 30(6), 846–857. — Breaching, and the hybrid breach/fill policy. ⚠️ **Retagged `[not-opened]` on 2026-09-14, on a human sign-off review.** `flow-routing.md` carries a five-sentence block quote attributed to this paper's §"When to breach and when to fill?", and that quote exists nowhere in this corpus but that one file — no register row covers it, no read log records the artefact being obtained, and a corpus-wide grep for `Soille` returns two hits, both inside flow-routing.md. The quote is retained and clearly marked unverified rather than deleted: it may well be accurate, and removing it would destroy the only record of where the claim came from. What is NOT retained is the implication that somebody here read it.
