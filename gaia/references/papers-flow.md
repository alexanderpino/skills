...
- **braun2013** `P` [not-opened] — Braun, J. & Willett, S.D. (2013). *A very efficient O(n), implicit and parallel method to solve the stream power equation governing fluvial incision and landscape evolution.* Geomorphology 180–181, 170–179. — The O(N) implicit solver. ⚠️ **Not obtained** — paywalled at Elsevier, no open copy reachable, so no locator into it is verified anywhere here. The scheme itself was verified second-hand in `cordonnier2016` eq. 2, which restates the implicit update and the root-to-leaves ordering. ⚠️ **This entry used to say that `cordonnier2016` §5 attributes both to Braun & Willett.** `stream-power.md` carries a checked retraction of exactly that: §5 states the scheme and the ordering requirement and credits nobody, while the attribution sits in §1 ("The original method from [BW13] is extended to efficiently model water flowing from lakes") and §4. The section number is dropped here rather than replaced, because the artefact could not be re-fetched to confirm the new one — `hal.science` serves a bot challenge for it — and a locator nobody has re-read is what produced the first error. [background]
