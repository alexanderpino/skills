...
| Boundaries blurry in exact proportion to the splatmap resolution | Both bands are a fixed fraction of the splatmap's bilinear ramp — 0.80 cross-faded, 0.55 with the additive bias, 0.19 with the weight-scaled one; `depth` sets the dither grain, not the band width | Raise the splatmap resolution. A height blend buys a narrower, grainier band, never independence — and *which* height blend decides how much: **4.2×** narrower with the weight-scaled bias `## Use this` prescribes (0.80 → 0.19), only **1.45×** with the additive one (0.80 → 0.55). This cell read a bare *4×* until 2026-09-15, which is the weight-scaled figure standing in for both |
