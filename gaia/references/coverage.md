...
- **resolution-independence** `covered` — **Which parameter of each operator has to rescale with cell size, and how.** The *principle* was already covered three times over — terrain-analysis-masks.md for slope thresholds, node-graph-runtime.md for the runtime rule that a parameter in cells is a bug and resolution belongs in the cache key, layering-filters-and-masks.md for filter radii and placement in metres. What none of them carried is the per-operator arithmetic those three assume, and this document is that: droplet reach, brush radius and the per-step rates; the pipe cross-section that ties wave speed to the cell rather than to the water; thermal pass count; saturation length as a floor rather than an exponent; and stream power's `K`, whose cell-size exponent is zero at exactly the default `m` and `n`. It also states the two verdicts a scaling table alone would hide — that slope and drainage-network topology cannot be made invariant at any price — and the refinement test that tells a preview which predicts from one that merely resembles. The single most common complaint against tools in this class. → resolution-independence.md
