...
- **resolution-independence** `covered` — **Which parameter of each operator has to rescale with cell size, and how.** The *principle* was already covered three times over — terrain-analysis-masks.md for slope thresholds, node-graph-runtime.md for the runtime rule that a parameter in cells is a bug and resolution belongs in the cache key, layering-filters-and-masks.md for filter radii and placement in metres. What none of them carried is the per-operator arithmetic those three assume, and this document is that: droplet reach, brush radius and the per-step rates; the pipe cross-section that ties wave speed to the cell rather than to the water; thermal pass count; saturation length as a floor rather than an exponent; and stream power's `K`, which in SI carries no cell-size exponent at all — the `Δx^(2m−n)` factor belongs to the mixed convention that keeps heights in metres while counting horizontal distance in cells, and it vanishes for any pair with `n = 2m`, the defaults among them. It also states the two verdicts a scaling table alone would hide — that slope and drainage-network topology cannot be made invariant at any price — and the refinement test that tells a preview which predicts from one that merely resembles. The single most common complaint against tools in this class. → resolution-independence.md
...
- **clastic-debris** `covered` — How does loose rock get onto a finished heightfield, and where does the instance path end? The Wentworth grade scale as the class vocabulary, Poisson-disk sampling whose one parameter is a separation and not a density, largest-first placement with cross-class rejection, and the measured budget that decides which classes become instances at all. → clastic-debris.md
- **procedural-placement** `planned` — Scattering **vegetation** from the masks: canopy radius, species masks, slope and moisture suitability, and clustering. Narrowed from "rocks, vegetation and debris" once `clastic-debris.md` took the rock half, which shares the sampler with this one and nothing else — a tree's constraint is a canopy footprint against a species mask, not an interpenetration test. Load-bearing for a tool builder and not yet written.
...
- **wave-spectra** `planned` — Which spectrum — Pierson–Moskowitz, JONSWAP, or a simplified fit — sampled at what `N` and `L`, normalised how, with `H_s = 4√m₀` as the acceptance test? `wave-models.md` names the family and stops. This row is where that acceptance test is **named**; `coastal-erosion.md` is the one document that **states** it, fixing `H_s = 4√m₀` with `m₀` the surface-elevation variance so it can choose between two longshore-transport constants written for different height conventions. That settles the convention for one coefficient, not for the synthesis — the sampling and the normalisation that produce the field are still unwritten, and normalisation is where a spectral ocean quietly comes out at the wrong height.
...
- **shader-craft** `covered` — **What must a heightfield shader get right that no general PBR or engine text teaches?** Screen-space derivatives across LOD seams and inside a divergent march, virtual-texture gradient scaling and feedback bias, explicit-LOD fetch semantics in a max-mip traversal, the depth-output direction under both depth conventions, divergent resource indices, and the fp16 hazards of `pow` and `normalize` on grazing terms. Scoped to what `physically-based-rendering` and `game-engine-guru` do not cover, and it says so at the top and points at both. Its load-bearing claim is that these are **language and API rules with cited spec text**, not folklore: four of its seven hazards are undefined behaviour by specification, all seven compile, and one of them costs only performance and is invisible in the image. The hazards were harvested out of the failure tables of the eleven rendering documents rather than invented, so those eleven can now cite one place. → shader-craft.md
...
- **isosurface-extraction** `out-of-scope` — Marching cubes and dual contouring over a signed distance field are the right tool when caves and overhangs are authored as volume, and a category error for a heightfield: they convert an exact 2.5-D function into an approximated 3-D one that then needs simplifying anyway (`mesh-extraction.md` says so where it lists what its method beats). Volumetric terrain is a different representation with its own literature, excluded here for the same reason `full-3d-fluid` is; the heightfield-to-mesh path is `mesh-extraction`.
- **display-transform** `out-of-scope` — Exposure, tonemapping and colour management (OCIO, ACES) belong to the `physically-based-rendering` skill: its reference map §7, *Scene integration & lighting*, owns colour management, physical light units with EV100 exposure, and post-processing. Gaia's rendering contracts stop at scene-referred linear radiance, pre-exposure — `atmosphere-and-aerial-perspective.md` types that output with its unit and white point — so the transform is applied once, downstream of the water composite and of aerial perspective, and never inside a Gaia document.
...
- **output-contracts** `planned` — What does a terrain pipeline hand downstream — every field a stage passes on, with its unit and its space (height in metres above which datum, normals in which handedness, flow in cells or in m³/s, masks in which range) — and what contract stops a heightfield meaning different things in two places? Acceptance test: one worked traversal, a single field followed from the node that produces it to the shader that reads it, with the unit and the space stated at every hop.
- **progressive-preview** `planned` — How does a coarse preview refine toward the build without popping, and how is it guaranteed to predict it? `node-graph-runtime.md` names "the preview tier" twice and defines it nowhere; this is the authoring tool's primary interaction loop, and it is the row that owes that definition — which fields are computed at reduced resolution, which solvers cannot be (the ones whose parameters do not scale with cell size, see `resolution-independence`), and what the preview may legitimately get wrong.
- **edit-history** `planned` — How does undo interact with a content-addressed cache of 67 MB evictable intermediates? `node-graph-runtime.md` already splits VALUE from TOPOLOGY invalidation, which is exactly the axis an undo stack records; this row asks what an undo entry stores (a parameter delta, a graph edit, or a cache key), what redo re-evaluates, and whether evicted intermediates make undo recompute or fail.
