...
sources:
  ...
  - { id: reed2015, tier: F, locator: "the section The Effects of Roundoff Error — the simulated indistinguishable-and-swap error table. It has SEVEN data rows, and THREE of them read 0% indist / 0% swap in both float32 columns: Unaltered Z values (control test), Reversed Z, and Infinite + reversed-Z. So reversed-Z with a float depth buffer is the only zero-error *mapping*, and scores zero in both of its rows, finite far plane and infinite — it is NOT the only zero-error row, the unaltered-Z control being the baseline the mappings are scored against, and Reed's own sentence is 'Reversed-Z with a float depth buffer gives a zero error rate in this test', with no 'only'. Cited here only for the storage evidence behind this corpus's reversed-Z mandate, which is what picks the depth token below" }
