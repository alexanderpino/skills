...
sources:
  ...
  - { id: wu2009, tier: P, locator: "READ AS LBNL-59102, the authors' technical report 'Two Strategies to Speed up Connected Component Labeling Algorithms', NOT the journal article; its Roman sections are cited here as Arabic. §1 and Figure 1(b) for the four-neighbour forward scan mask; §1 category 2 of five for the three phases of a two-pass labeller — there is no §1.1; §4.2 Theorem 3 — 'the total time required by a two-pass algorithm using any union-find with path compression is O(p), where p is the number of pixels in the 2D image'; §4.1 Theorem 1 for the decision tree, which claims MINIMALITY of neighbours visited and no factor at all, with the quantity in §4.3 Theorem 4 — 7/3 neighbours on average against 4, an expected speedup of 12/7 or about 1.7 per §5.2, not a factor of two" }
...
**The algorithm** [wu2009] §1, category 2 of five: one raster pass assigns each foreground cell a provisional label,
