...
sources:
  ...
  - { id: houdini_hdk, tier: F, locator: "HDK 'Data Dependencies' (NOT the 'Cooking' page, which carries none of this) — the OP_Node::addExtraInput() requirement for data cooked 'via some other means', and the statement that extra inputs are cleared as soon as they are traversed on dirty propagation, so they must be re-registered on every cook" }
