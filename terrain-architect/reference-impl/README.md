...
| `hero.py` — **3D hero view** | a pure-numpy software rasteriser (perspective camera + z-buffer) | projects the heightfield as a mesh, drapes it with the substance colouring, lights it in 3D (sun+sky + AO), sky background + depth fog + supersampled AA → `hero.png`. The angled 3/4 "viewport" view (vs the top-down montages); no 3D deps, just numpy + the PNG writer |
