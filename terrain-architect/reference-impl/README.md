...
| `tectonics.py` — faults (scarps + fault-as-K) | `02` | `fault_scarp`: the `faultIteration` displacement fractal (feathered offsets, decaying displacement → fault blocks; a structural detail layer, no drainage). `fault_weakness`: the geologically-correct coupling — an erodibility field with the fault traces **more erodible**, fed to `stream_power_evolve` so valleys follow structure. `tests/test_tectonics.py`: finite/deterministic scarp; the decisive check — fault‑weakened K makes fault cells become valleys far above chance |
| `aeolian.py` — wind abrasion (yardangs) | `16` Ward & Greeley 1984 | `yardang`: carves streamlined ridges **‖ the wind** from a soft substrate — anisotropic-fBm lanes abraded with a **bottom-weighted** saltation term (sand saltates ~1 m up → undercut bases), confined to a soft mask. F-tier (`tests/test_aeolian.py`): ridges elongate along-wind, carve-only, soft-only, low ground cut fastest |
