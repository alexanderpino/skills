...
| `crater_demo.py` — natural render (presentation) | composites the `crater.py` physics with `noise.py` (01) | irregular rims, slumped/terraced walls, lumpy central massif, streaky ejecta **rays**, grazing **plow furrow** (deepens downrange, levees, forward rampart); hillshaded → `crater_matrix.png`. A *look*, not oracle-verified |
