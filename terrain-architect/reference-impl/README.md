...
| `crater.py` — parameterised impact | `11` size + **angle**: Collins/Melosh/Marcus 2005 π-scaling, Gault–Wedekind obliquity | `D_tc ∝ L^0.78 v^0.44 g^(−0.22) (sinθ)^(1/3)` (exponents exact); transition `∝ 1/g`; simple↔complex; circular above ~12°, elongated below; **mass-conserving** ejecta pushed downrange (up-range forbidden zone / butterfly). `crater_demo.py` → `crater_matrix.png` cut/fill matrix |
