...
| `landforms.py` — geological landforms | `11` impact craters (Pike/Melosh), strata & terracing, folding, karst sinkholes, **fault-block buttes**, **mountain generator** | crater `depth/D ≈ 0.2`, rim raised, ejecta `∝ r⁻³`, central peak when complex, diameter `∝ g` inversely; strata periodic; terrace snaps to treads; fold is a sinusoid; karst carves pits **only** on soluble rock (the `03` do-not-fill exception). `fault_block_butte` & `mountain` are **feature primitives** (Génévaux 2015 / Guérin 2016) — the Gaea-style "Mountain" node: a ridge-crest skeleton + ridged detail you place, combine and erode (not thresholded noise). `python landforms.py` → `landforms.png` (raw \| eroded) |
