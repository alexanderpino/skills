...
| `hydrology.py` — **water surface** | Leopold & Maddock 1953 hydraulic geometry + priority-flood | turns a **discharge** field (m³/s, from the flow sim or the `rain·area` proxy) into a water LEVEL: lakes fill depressions to spill level, rivers get a discharge-scaled depth; `water_depth = surface − bed`. The rendering layer over the real flow |
