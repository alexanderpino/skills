...
| `landforms.py` — geological landforms | `11` impact craters (Pike/Melosh), strata & terracing, folding, karst sinkholes, **fault-block buttes** | crater `depth/D ≈ 0.2`, rim raised, ejecta `∝ r⁻³`, central peak when complex, diameter `∝ g` inversely; strata periodic; terrace snaps to treads; fold is a sinusoid; karst carves pits **only** on soluble rock (the `03` do-not-fill exception); `fault_block_butte` is a feature-primitive (Génévaux 2015 / Guérin 2016) on a joint-bounded `sd_convex_polygon` footprint — flat top, cliff, talus break |
...
| `hydrology.py` — **water surface** | Leopold & Maddock 1953 hydraulic geometry + priority-flood | water is a substance at a LEVEL, not a colour: lakes fill enclosed depressions to their spill level, rivers carry a discharge-scaled depth in their carved channels; `water_depth = surface − bed`. Feeds a real water render, not a blue decal |
| `hero.py` — **3D hero view** | a pure-numpy software rasteriser (perspective camera + z-buffer) | projects the heightfield as a mesh, lights it in 3D (sun+sky + AO), draped with substance colour + real `hydrology` water, sky + depth fog + supersampled AA → `hero.png`. `from_graph()` renders terrain **straight from the node graph** (uplift → stream-power erosion → thermal → hydrology), no hand-sculpting. The angled 3/4 "viewport" view; no 3D deps |
