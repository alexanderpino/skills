...
| Colorizer / splat | `analysis.derive_substances` + `material_rgb` (default — the 7-channel stack and `render._SUBSTANCE_PALETTE` are both keyed to `analysis.SUBSTANCE_NAMES`, and the pairing is run end-to-end by `tests/test_render.py::test_the_default_colorizer_pairing_runs`, because it once was not and raised); `render.satmap`/`splat_blend` (toolbox); `render.extract_satmap` (authors a SatMap from real satellite imagery — Gaea's own SatMap-library path; `SATMAPS["desert_terra"]` is extracted from a PD NASA Terra image); `photoreal` | **Andersson/Frostbite 2007** splat; **substance placement** by slope/aspect/curvature/flow | splatmap masks (SatMap CLUT also available) |
