...
|---|---|---|---|---|
| **Fluvial (large-scale)** | stream-power incision — `erosion_streampower.stream_power_evolve` (Braun & Willett 2013 O(N) implicit solver, ×-validated vs Landlab) | Cordonnier 2016 uplift+SPL; Schott 2023 interactive; McDonald & Cordonnier 2026 momentum particles | **SOTA core** — but **detachment-limited: incises, never deposits** | add a transport-limited / sediment-flux closure (see deposition) |
...
| **Tectonic / orogeny** | isostasy + SPL uplift + **fault scarps, fault-as-K & a Voronoi plate sim** (`tectonics.fault_scarp`, `tectonics.fault_weakness`, `tectonics.plate_uplift`) | Cordonnier 2016 uplift+fluvial; Sculpting Mountains 2018 | **SOTA & ahead of all commercial tools** | foreground it; optional flexural isostasy / folds |
| **Thermal / hillslope** | Musgrave 1989 angle-of-repose talus (`erosion_thermal.thermal_erosion`, double-buffered and mass-conserving, with the distance correction that keeps a talus cone radial) **+ linear hillslope diffusion coupled inside the SPL solver** (`erosion_streampower.stream_power_evolve(D=)`, Cordonnier's companion term; scalar or field `D`) | repose (rock) + linear diffusion (Culling) **+ nonlinear diffusion** Roering 1999 (soil creep) | **solid**; linear coupling done, nonlinear still missing | add the Roering 1999 nonlinear (slope-dependent) closure beside the linear term |
...
| **Flow routing / hydrology** | `flow.d8_accumulation` + `flow.mfd_accumulation` + `flow.priority_flood_fill` (× RichDEM/pysheds/Landlab), and **the hybrid router `03` recommends ships**: `flow.hybrid_accumulation` — MFD on the hillslope, D8 once a cell's own accumulated area has channelised, decided inside **one** pass rather than spliced from two finished rasters (the splice creates water: 1.58× D8's total on one 160×160 DEM; the router returns 1.000000000000 of the domain area at the outlets). Tested in `tests/test_flow_anatomy.py`, drawn as `flow_anatomy` panel c, and selectable from the node graph | + D-infinity (Tarboton 1997); Fill-Spill-Merge (Barnes 2020) for real lakes; FastFlow 2024 | **current / solid** | optional D-∞; **Fill-Spill-Merge** for real lakes vs filled-flat |
...
| **Debris runout** | `runout.voellmy_runout` (Voellmy 1955, integrated in space via the work-energy form; the Coulomb limit reproduces the reach angle tan α = μ) | Jain/Beneš/Cordonnier 2024 debris-flow | **solid** | frontier watch |
