...
def wind_redistribute(h, snow, wind, *, cellsize=1.0, rate=0.4, iters=3, speed_ref=None):
    ...
    PRODUCTION CAVEATS, measured 2026 (see 13) — both still apply to the field-aware form below:
    the destination index wraps modularly (`% n`, `% m`), teleporting scoured mass to the far edge
    of the tile - clamp in production. And the fixed one-cell deposit assumes the snow surface
    tracks the bedrock; on deep settled drifts that decouple from it, parcels land on still-windward
    ground and the face-integrated effect INVERTS (windward +9.3% measured). Production form: walk
    each parcel downwind to the first sheltered cell (surface no longer rising into the wind), cap
    the walk in metres, and cap the transportable layer per pass."""
