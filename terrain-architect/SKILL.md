...
| `reference-impl/` | **Runnable, pytest-verified** numpy mirrors of the sim pseudocode (procedural noise, droplet/pipe/thermal/stream-power erosion, flow routing, diffusion, dunes, isostatic flexure, mass-consistent wind, Voellmy runout, tephra/age-depth/PDC/avulsion, analysis/masks/materials, primitives/ops/filters, scatter, geological landforms), each checked against its `09` oracle. They are executable specifications for an owned implementation, not runtime dependencies; optional tests cross-validate flow operations against RichDEM and pysheds, and stream power, D8 accumulation and hillslope diffusion against Landlab (MIT). A dependency-free **graph+render sandbox** (`reference-impl/graph_demo.py`, `reference-impl/render.py`) wires these nodes into a Legal-Order DAG — a scale model of `14`'s substrate with content-addressed caching — and renders the `09` review modes to PNG, for testing algorithms by eye during development. Provenance and licences per node: `reference-impl/GROUNDING.md` |
