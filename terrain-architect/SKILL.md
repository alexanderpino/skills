...
grids*, for the choice; `references/26-hexagonal-lattice.md` for the implementation (kernels,
constants, triangulation, sampling). This is entirely separate from — and applies with or without —
the spherical-grid choice a *planet* forces (`08`/`25`); a hex grid is for flat terrain first, the
sphere only later.
...
| `references/05-erosion-thermal-aeolian.md` | Thermal/talus, mass wasting (landslides, debris flows), wind transport, Werner dune model, anchored/obstacle dunes (echo, climbing, falling, sand ramps, shadow dunes), dune hierarchy (draa), vegetation-anchored dunes (parabolics, blowouts) |
...
| `references/08-output-contract.md` | Field contract, precision, tiling, aprons, seams, **hexagonal grids (a grid system in their own right — optimal sampling lattice, D6 routing, no D4/D8 √2 ambiguity)**, planetary/spherical domains (cube-sphere, icosahedral hexagonal DGGS / Goldberg polyhedron, HEALPix, seam routing), DEM & sensor realism (hydro-enforcement, void-fill, SAR/lidar artefacts, error models), LOD, clipmaps, splatmaps, satmaps, normal/AO map encoding |
