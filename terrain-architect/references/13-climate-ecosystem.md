...
windField(h, baseDir, baseSpeed):
    for each cell p:
        ...
        #    The fetch argument is load-bearing: a LOCAL gradient is maximal at the windward
        #    inflection and exactly zero at the crest, so it puts the wind maximum on the flank.
        #    A max over intermediate samples does not rescue it — use the fetch secant (see below).
