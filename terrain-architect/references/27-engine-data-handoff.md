...
| `insolation` | [0,1] received fraction | Horizon-angle sweep (Timonen & Westerholm, `06`) or explicit raymarch toward sampled sun positions over the seasonal arc | Terrain self-shadowing of solar radiation. Dictates where snow **melts** (sun-facing slopes — south-facing in the northern hemisphere) and where it **persists** (deep ravines, pole-facing walls that never see the sun). Distinct from `ao`: AO integrates the whole sky, insolation integrates the sun's arc. Do not substitute one for the other. |
...
| Map | Unit / range | Producer | Notes |
|---|---|---|---|
| `wetness` | state: [0,1] saturation; derived companion: TWI (`06`) | Hydraulic sim residual water + infiltration (state); Beven & Kirkby TWI (derived) | **Puddling** — where water settles in the soil after flowing, not merely where it flowed. Drives mud materials, reeds and wetland vegetation (`13`), engine-side puddle shaders and rain response. Ship state and derived components separately (above). |
| `flowVelocity` | m/s, 2D vector | Routing discharge + hydraulic geometry (`03`), or the pipe sim's own velocity field (`04`) | With `waterDepth`, the complete driving data for engine-side fluid mechanics: flow-map shaders, SPH/FLIP seeding, foam *generated by the engine* where the tool's data says the water is fast and shallow. The tool ships the vectors; it never ships the foam (`SKILL.md`). |
...
- **Co-evolution budget.** Run any erosion node with soil tracking on a closed domain: total
  ...
  and measured (`09`, mass conservation). A node that changed height without touching the maps
  its process implies fails review by inspection — the output-port list is the tell.
