...
| `liquidBody[i]` | per-body record (not a raster) | Optical + rheological identity (`28`) | **What the liquid *is*.** Absorption/scattering per channel, beam vs diffuse attenuation, scatter colour, Secchi depth, Jerlov/Forel-Ule class; plus yield stress and viscosity for non-water liquids. The engine's water shader has no other source for `sigma` — without this it falls back to one global constant and every body looks alike. Ship the causal state alongside the derived optics so the values can be re-derived and checked. |
