...
| `liquidBody[i]` | per-body record | Record list, not a raster | **The liquid's identity** (`28`), one per lake / river reach / sea / lava flow: a `bodyType` tag (sea / lake / pond / river / stream / estuary / wetland, `03`) so the engine selects the right surface model — lake gets wind waves only, river gets flow, sea gets swell + tide; causal state (`chl`, `cdom_a440`, `spm`, lithology class, mean depth), rheology (`viscosity`, `yield_stress`, `shear_index`, `emission_temperature`), derived optics (`ior`, `a_RGB`, `b_b_RGB`, `c_RGB`, `K_d_RGB`, scatter colour), the `fetch`/exposure field for enclosed water (the wind-wave driver, `12`), and QA fields (`secchi_depth_m`, `jerlov_type`, `forel_ule_index`). The engine builds its per-body `sigma` and scatter colour from this — ocean, clear lake and turbid river must never share one global constant (terrain-renderer `12`) |
