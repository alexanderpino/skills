...
and the C4-vs-C6 verification signatures. **Symbol warning when crossing:** this file writes `s`
for the hexagon *circumradius* (the coordinates section above) and `d` for the *centre-to-centre
neighbour spacing* in the stencil formulas (`d = √3·s`); `26` writes `s` for that same
centre-to-centre spacing — so `26`'s `s` is this file's `d`, not its `s`. The `cellSize` contract —
centre-to-centre spacing — is identical everywhere; the formulas are not interchangeable without
converting symbols.
