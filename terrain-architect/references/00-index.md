...
| D6 / MFD flow routing on a hex mesh | P | Liao et al. 2020 (HexWatershed); Liao et al. 2025 (ISEA equal-area) — both **mesh/DGGS**. The **planar-lattice** literature is separate: Wang, Ai, Shen & Li 2020 (*Transactions in GIS* 24(2):483–507) names the cause — square eight-neighbourhood grids "have inconsistent distance measurements between orthogonal and diagonal neighborhoods, resulting in valley lines extracted by the D8 algorithm not guaranteeing isotropy characteristics" — with the D8 comparison in Wang & Ai 2018 (ISPRS Archives XLII-4:687–692). **Neither introduced D6**: de Sousa, Nery, Sousa & Matos 2006 (Accuracy 2006) already routes steepest-descent on a hex-resampled DEM. All of them are **single-receiver** and are drainage-*structure* results, so they ground routing and nothing downstream of it. Landlab ships hex FlowDirectorSteepest/MFD; **D8 and D∞ do not exist there** (`03`, `26`) |
| CG erosion on a hex heightfield | ? (literature) / F (any port) | **A verified absence, and the most load-bearing one here: every hex erosion kernel is F-tier by it.** No graphics paper running hydraulic, thermal or stream-power erosion on a hexagonal heightfield could be *verified to exist* — weaker than proof of absence, and all that is claimed. Near misses, named so the gap stays legible: Tucker, McCoy & Hobley 2018 (*ESurf* 6:563–582) evolves hillslopes on a hexagonal lattice but as a grain-state CA in geomorphology, with no height array being eroded; Liao et al. 2020 routes on hex meshes with no erosion at all. A hex erosion port is an engineering translation standing on P-tier hydrology, which does not lend it its tier. **Do not imply a paper** (`26`, `04`, `05`) |
