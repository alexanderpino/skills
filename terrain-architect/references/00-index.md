...
| Side-channel mask registry ("accumulator") | F | The sim→splatmap fan-in as one node. Two non-discretionary rules, both consequences of `14`'s purity contract: (1) its input set is **undeclared**, so the Merkle key misses it — adding a simulation leaves the key unchanged and every consumer serves a **stale, silently-wrong** splatmap; desugar at plan time and hash the *resolved* `(producerId, outPort)` set into the key. (2) Overlaps need a **documented total order** (snow → water → vegetation → debris → base), never a bare `max` (`10`) and never traversal order, which would make output depend on node insertion order. It is `LOCAL` but its cone is the whole graph, so it inherits its worst producer's tiling contract — and it is the natural single place to assert `Σ masks ≤ 1` on the **raw coverage masks** (`≤`, not `=`: the shortfall is the base material's share, and `08`'s "splat weights must sum to 1" is the object one stage later, after compositing fills the remainder). The assertion belongs *here* because an ordered over-composite makes over-subscription undetectable downstream — masks summing to 1.8 still yield effective weights summing to 1.0000000000 (`06`, `09`, `14`) |
