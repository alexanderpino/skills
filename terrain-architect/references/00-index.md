...
| Surf-zone morphodynamic loop (radiation stress → currents → transport → Exner) | P (loop) / L (realisation) | Radiation stress: Longuet-Higgins & Stewart 1962 (JFM 13(4), 481–504) for priority, **1964** (Deep-Sea Res. 11(4), 529–562) for the nearshore set-up/circulation exposition; longshore current Longuet-Higgins 1970 (JGR 75(33), two parts). Sediment continuity is the **classical Exner** equation — Paola & Voller 2005 is the *generalisation* (uplift, compaction, dual interfaces), not the plain form (`12`). **Reviewed against an implementation 2026-08** — two corrections inline in `12`: the runnable core's `min(shoal, γ·d)` cap is memoryless and cannot reform a wave (replace with an energy-flux march with `γ_b`/`γ_s` hysteresis, Dally, Dean & Dalrymple 1985), and the longshore `∝` now carries its derived coefficient `5π/16` with the alongshore-refraction term that is 25% of it |
| Undertow & breakpoint bar (bar crest at depth ≈ `H_b/γ`; storm/calm migration) | P | Svendsen 1984, Coastal Eng. 8(4), 347–365 (undertow); Bailard 1981, JGR 86(C11), 10938–10954 (energetics total load — distinct from Bailard & Inman 1981, JGR 86(C3), the bedload paper) (`12`). **Reviewed against an implementation 2026-08**: the crest depth `≈ H_b/γ` was attacked across five sea states and **stood** (the apparent shortfall is a grid artefact, not a missing term); the *mechanism* was corrected — deleting the undertow still produces a bar at the same depth, so the undertow sets the bar's **relief and its trough**, not its position (`12`). **Corrected again 2026-08**, one level down: breaking does not *destroy* the wave's third moment, it **rotates** it — `Sk² + As²` is a function of the shape parameter alone (`D`), so a transport carrying only the skewness reads **half** of what its own wave shape has. The bar-position result stands; the collapse is slower than the `(1 − f_brk)` straight line an implementation reaches for, by **29.3% at half breaking** (`12`) |
...
| Filtered `waterDepth` copy for engine wave synthesis — **filter at the grid-noise scale, not the wavelength scale** | P (failure mode) | `27` — **corrected 2026-08 against an implementation.** A wavelength-scale filter (`L₀/10` = 12.6 m) removes 68% of an 11 m breaker bar and moves the break point 58 m shoreward; inside a morphodynamic loop (`12`) the depth field is a *state variable* and filtering it at the wave's own scale disables the feedback that limits bar growth. Wavelength-scale is right only for a cosmetic break/foam **mask** |
