...
| Hexagonal grid (planar working grid, in its own right) | P (sampling) / F (coords) | The grid-system case: 6 equidistant edge-neighbours ⇒ **no D4/D8 √2 ambiguity**, D6 flow routing, more isotropic CA/diffusion; axial/cube/offset coords (Red Blob Games, F). A grid system, not the square raster's poor cousin. Sampling-optimality citations and implementation depth (kernels, constants, triangulation, mips): the "Hexagonal sampling lattice" rows below → `26` (`08`, `09`) |
...
| Hexagonal sampling lattice (flat heightfields) | P | Petersen & Middleton 1962 (disc-band-limited optimality, exact 13.4% = 1−√3/2); Mersereau 1979; Middleton & Sivaswamy 2005 (book). Grid-system framing & when-to-choose: the "Hexagonal grid" row above → `08` (`26`) |
