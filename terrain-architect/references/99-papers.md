...
- **Longuet-Higgins, M.S. & Stewart, R.W. (1962).** *Radiation stress and mass transport in gravity
  waves, with application to 'surf beats'.* Journal of Fluid Mechanics **13**(4), 481–504. — Coins
  and derives the radiation-stress concept: waves carry momentum whose convergence at breaking
  drives setup and the nearshore circulation — the engine of the surf-zone loop (`12`). Cite for
  priority; cite the 1964 paper below for the nearshore applications.
- **Longuet-Higgins, M.S. & Stewart, R.W. (1964).** *Radiation stresses in water waves; a physical
  discussion, with applications.* Deep-Sea Research and Oceanographic Abstracts **11**(4),
  529–562. — The general physical exposition that develops wave set-up and wave–current
  interaction. **This is the better primary citation** when the claim is about set-up or nearshore
  circulation rather than about priority (`12`).
- **Longuet-Higgins, M.S. (1970).** *Longshore currents generated by obliquely incident sea waves,
  parts 1 & 2.* Journal of Geophysical Research 75(33), 6778–6789 & 6790–6801. — Radiation stress
  applied: alongshore thrust `(E₀/4)·sin 2θ₀` **in deep-water quantities**, and the
  longshore-current profile across the surf zone (part 2). The current that executes CERC transport
  (`12`). *Watch the coefficient:* it carries `c_g/c`, which is ½ in deep water and 1 at breaking,
  so the same flux is `(E_b/2)·sin 2θ_b` in breaking-zone terms — pairing ¼ with breaking-zone
  values is a factor-of-two error.
- **Bowen, A.J. (1969).** *Rip currents: 1. Theoretical investigations.* Journal of Geophysical
  Research 74(23), 5467–5478. — Alongshore variation in breaker height → setup gradients → feeder
  currents converging into a seaward rip jet; the theoretical basis of bar–rip circulation (`12`).
- **Svendsen, I.A. (1984).** *Mass flux and undertow in a surf zone.* Coastal Engineering 8,
  347–365. — Shoreward mass flux above trough level (drift + roller) balanced by the seaward
  undertow below; the offshore-directed half of the breakpoint-bar convergence (`12`).
- **Bailard, J.A. (1981).** *An energetics total load sediment transport model for a plane sloping
  beach.* Journal of Geophysical Research 86(C11), 10938–10954. — Bagnold energetics on a beach:
  transport parallel to the instantaneous velocity plus a downslope gravity term; the standard
  cross-shore transport closure (`12`). Because the transport moments are *odd*, net onshore
  transport requires velocity **skewness/asymmetry** — a symmetric wave moves no net sand, which
  is why an implementation using orbital amplitude alone is wrong. **Do not confuse** with
  Bailard & Inman (1981), *An energetics bedload model for a plane sloping beach: local
  transport*, JGR 86(C3), 2035–2043 — different paper, two authors, bedload only.
- **Wright, L.D. & Short, A.D. (1984).** *Morphodynamic variability of surf zones and beaches: a
  synthesis.* Marine Geology 56(1–4), 93–118. — The beach-state model: `Ω = H_b/(w_s·T)` classifies
  reflective (<1), intermediate bar–rip (1–6), dissipative (>6). Which surf zone to author, from
  data (`12`).
- **MacMahan, J.H., Thornton, E.B. & Reniers, A.J.H.M. (2006).** *Rip current review.* Coastal
  Engineering 53(2–3), 191–208. — Field synthesis of rip kinematics, spacing and morphodynamic
  feedback. Characteristic spacing is **O(100 m)** (field values typically 50–500 m) and
  **quasi-rhythmic** — a preferred wavelength with scatter, not true disorder (`12`).
- **O'Brien, M.P. (1969).** *Equilibrium flow areas of inlets on sandy coasts.* Journal of the
  Waterways and Harbors Division, ASCE 95(WW1), 43–52. — The tidal prism–inlet area relation
  `A_inlet = C_OB·P_tide^n`, `n ≈ 0.85` (`C_OB` = 4.69×10⁻⁴ over 28 US entrances, 1.08×10⁻⁴ over
  the 8 unjettied ones, ft units); the scour feedback that keeps inlets open and sizes their
  deltas (`12`). First form: **O'Brien (1931)**, *Estuary tidal prisms related to entrance areas*,
  Civil Engineering 1, 738–739. **Do not merge** with the similarly-titled O'Brien (1966),
  *Equilibrium flow areas of tidal inlets on sandy coasts*, 10th ICCE, 676–686 — a different item.
- **Paola, C. & Voller, V.R. (2005).** *A generalized Exner equation for sediment mass balance.*
  Journal of Geophysical Research — Earth Surface 110, F04014. — **Generalizes** sediment
  continuity to include tectonic uplift/subsidence, soil formation and creep, compaction, and
  chemical precipitation/dissolution, with the bedrock–sediment and sediment–flow interfaces
  evolving independently. Cite it for *those* terms. The plain `∂h/∂t = −∇·q/(1−poros)` form that
  closes the surf-zone and fluvial loops is the **classical Exner** equation and predates this
  paper — attributing the simple form here is a misattribution (`12`, `04`).
